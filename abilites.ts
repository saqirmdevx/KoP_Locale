...
const _getSpellDescriptionLang = (
  ...
  switch (id) {
    ...
    case SpellList.KIRA_AUTOATTACK: {
      ...
      return {
        en: `Kira fires a spark, dealing ${basic_damage}. \n\n<b>Enhanced: (Lightning Shock)</b> Kira also casts a lightning bolt at the nearest hero. If Lightning Shock hits an ally, it <c:heal>restores ${enh_damage_heal} health</c:heal> and grants ${bonusAttackSpeedValue} <c:attackspeed>bonus attack speed</c:attackspeed> and ${bonusMovementSpeed} <c:movespeed>bonus movement speed</c:movespeed> for a short duration.`,
        br: `Kira dispara uma faísca, causando ${basic_damage}. \n\n<b>Aprimorado: (Choque Elétrico)</b> Kira também lança um raio no herói mais próximo. Se Choque Elétrico atingir um aliado, ele <c:heal>restaura</c:heal> ${enh_damage_heal} de vida e concede ${bonusAttackSpeedValue} de <c:attackspeed>bônus de velocidade de ataque</c:attackspeed> e ${bonusMovementSpeed} de <c:movespeed>bônus de velocidade de movimento</c:movespeed> por um curto período.`,
        ru: `Кира выпускает искру, нанося ${basic_damage}. \n\n<b>[Усиленная]: (Удар молнии)</b> Кира также выпускает молнию в ближайшего героя. Если Удар молнии попадает в союзника, он <c:heal>восстанавливает ${enh_damage_heal} здоровья</c:heal> и даёт ${bonusAttackSpeedValue} <c:attackspeed>бонуса к скорости атаки</c:attackspeed> и ${bonusMovementSpeed} <c:movespeed>бонуса к скорости передвижения</c:movespeed> на короткое время.`,
        cz: `Kira vystřelí jiskru, která způsobí ${basic_damage} poškození. \n\n<b>[Vylepšené]: (Bleskový šok)</b> Kira navíc sesílá blesk na nejbližšího hrdinu. Pokud Bleskový šok zasáhne spojence, <c:heal>obnoví</c:heal> mu ${enh_damage_heal} zdraví a poskytne ${bonusAttackSpeedValue} <c:attackspeed>bonus k rychlosti útoku</c:attackspeed> a ${bonusMovementSpeed} <c:movespeed>bonus k rychlosti pohybu</c:movespeed> na krátkou dobu.`,
        zh: `奇菈射出一道電光，造成 ${basic_damage}。 \n\n<b>[強化後]：(閃電衝擊)</b> 奇菈還會向最近的英雄施放閃電。如果閃電衝擊命中盟友，將<c:heal>恢復 ${enh_damage_heal} 生命</c:heal>，並提供 ${bonusAttackSpeedValue} <c:attackspeed>額外攻擊速度</c:attackspeed>與 ${bonusMovementSpeed} <c:movespeed>額外移動速度</c:movespeed>，持續短時間。`,
        fr: `Kira tire une étincelle, infligeant ${basic_damage}. \n\n<b>Amélioré : (Choc électrique)</b> Kira lance également un éclair sur le héros le plus proche. Si le Choc électrique touche un allié, il <c:heal>restaure</c:heal> ${enh_damage_heal} points de vie et accorde ${bonusAttackSpeedValue} de <c:attackspeed>bonus de vitesse d'attaque</c:attackspeed> ainsi que ${bonusMovementSpeed} de <c:movespeed>bonus de vitesse de déplacement</c:movespeed> pendant une courte durée.`,
        vi: `Kira bắn ra một tia điện, gây ${basic_damage}. \n\n<b>Cường hóa: (Điện Kích)</b> Kira đồng thời phóng một tia sét vào anh hùng gần nhất. Nếu Điện Kích trúng đồng minh, nó <c:heal>hồi ${enh_damage_heal} máu</c:heal> và cho ${bonusAttackSpeedValue} <c:attackspeed>thêm tốc độ đánh</c:attackspeed> cùng ${bonusMovementSpeed} <c:movespeed>thêm tốc độ di chuyển</c:movespeed> trong một thời gian ngắn.`,
        id: `Kira menembakkan percikan listrik, memberikan ${basic_damage}. \n\n<b>Enhanced: (Lightning Shock)</b> Kira juga melepaskan petir ke hero terdekat. Jika Lightning Shock mengenai sekutu, itu <c:heal>memulihkan ${enh_damage_heal} health</c:heal> dan memberikan ${bonusAttackSpeedValue} <c:attackspeed>bonus kecepatan serangan</c:attackspeed> serta ${bonusMovementSpeed} <c:movespeed>bonus kecepatan bergerak</c:movespeed> untuk durasi singkat.`,
        kr: `키라가 스파크를 발사하여 ${basic_damage}의 피해를 입힙니다. \n\n<b>강화: (전격 충격)</b> 키라가 가장 가까운 영웅에게 번개도 발사합니다. 전격 충격이 아군에게 명중하면 ${enh_damage_heal}의 체력을 <c:heal>회복</c:heal>시키고 짧은 시간 동안 ${bonusAttackSpeedValue}의 <c:attackspeed>추가 공격 속도</c:attackspeed>와 ${bonusMovementSpeed}의 <c:movespeed>추가 이동 속도</c:movespeed>를 부여합니다.`,
      }
    }
