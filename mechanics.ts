...
const _getMechanicDescriptionLang = (id: MechanicId): { [key in string]: string } => {
  switch (id) {
    ...
    case MechanicId.CC_EFFECTS: {
      ...
      return {
        en: `<b>Crowd control (CC)</b> limits movement or actions. Most effects can be <c:dispel>dispelled</c:dispel>. <b>Tenacity</b> reduces their duration.<ul><li>${stunIcon} <c:stun>Stun:</c:stun> stops movement, attacks, and abilities. Interrupts your current cast.</li><li>${rootIcon} <c:control>Root:</c:control> stops movement and attacks. You can still cast abilities.</li><li>${silenceIcon} <c:control>Silence:</c:control> stops abilities. You can still move and attack.</li><li>${freezeIcon} <c:control>Freeze:</c:control> same as Root - no movement and no attacks, but abilities still work.</li><li>${slowIcon} <c:slow>Slow:</c:slow> reduces movement speed.</li><li>${fearIcon} <c:control>Fear and Charm:</c:control> force you to move in a direction you cannot choose, and block your attacks and abilities.</li><li><c:control>Knockback:</c:control> launches you into the air and stuns you for its duration.</li><li><c:control>Pull:</c:control> drags you and stops you acting while it lasts.</li><li><c:control>Pushback:</c:control> shoves you, but you can keep attacking and casting.</li></ul><b>Hard CC</b> means <c:stun>Stun</c:stun>, <c:control>Charm</c:control>, and <c:control>Fear</c:control>. Immunity to hard CC protects against these three.`,
        ru: `Контроль — это всё, что отнимает у героя возможность действовать. Большую его часть можно <c:dispel>снять</c:dispel>, а <b>стойкость</b> сокращает его длительность на вас.<br/><br/>• ${stunIcon} <c:stun>Оглушение</c:stun> — ни движения, ни атак, ни способностей, текущий каст прерывается<br/>• ${rootIcon} <c:control>Обездвиживание</c:control> — вы не можете ни двигаться, ни атаковать, но способности остаются доступны<br/>• ${silenceIcon} <c:control>Немота</c:control> — нет способностей, но движение и атаки доступны<br/>• ${freezeIcon} <c:control>Заморозка</c:control> — то же, что обездвиживание: ни движения, ни атак, но способности работают<br/>• ${slowIcon} <c:slow>Замедление</c:slow> — снижение скорости передвижения, а не полная блокировка<br/>• ${fearIcon} <c:control>Страх</c:control> и <c:control>Очарование</c:control> — вы продолжаете двигаться, но направление выбирают за вас, а атаки и способности заблокированы<br/>• <c:control>Отбрасывание</c:control> — подбрасывает вас в воздух и оглушает на всю длительность<br/>• <c:control>Притягивание</c:control> — тащит вас и не даёт действовать<br/>• <c:control>Толчок</c:control> — сдвигает вас, но атаковать и применять способности вы можете<br/><br/><b>Оглушение, Очарование и Страх</b> считаются <b>жёстким контролем</b>; эффекты и предметы, дающие иммунитет к жёсткому контролю, имеют в виду именно их.`,
        cz: `Ovládání je všechno, co hrdinovi bere možnost jednat. Většinu lze <c:dispel>rozptýlit</c:dispel> a <b>houževnatost</b> zkracuje, jak dlouho na tebe působí.<br/><br/>• ${stunIcon} <c:stun>Omráčení</c:stun> – žádný pohyb, útoky ani schopnosti, a rozseslané kouzlo se přeruší<br/>• ${rootIcon} <c:control>Znehybnění</c:control> – nemůžeš se hýbat ani útočit, ale schopnosti pořád sesíláš<br/>• ${silenceIcon} <c:control>Umlčení</c:control> – žádné schopnosti, ale pohyb i útok fungují<br/>• ${freezeIcon} <c:control>Zmrazení</c:control> – stejné jako znehybnění: žádný pohyb ani útoky, ale schopnosti fungují<br/>• ${slowIcon} <c:slow>Zpomalení</c:slow> – snížená rychlost pohybu, ne úplné zablokování<br/>• ${fearIcon} <c:control>Strach</c:control> a <c:control>Okouzlení</c:control> – dál se pohybuješ, ale směr za tebe vybírá někdo jiný a útoky i schopnosti jsou zablokované<br/>• <c:control>Odhození</c:control> – vymrští tě do vzduchu a na celou dobu omráčí<br/>• <c:control>Přitažení</c:control> – táhne tě a nedovolí ti jednat<br/>• <c:control>Odstrčení</c:control> – odsune tě, ale útočit i sesílat můžeš dál<br/><br/><b>Omráčení, Okouzlení a Strach</b> se počítají jako <b>tvrdé ovládání</b>; efekty a předměty mluvící o imunitě vůči tvrdému ovládání myslí právě tyto tři.`,
        br: `Controle de grupo é tudo que tira as ações de um herói. A maior parte pode ser <c:dispel>dissipada</c:dispel>, e <b>tenacidade</b> encurta quanto tempo dura em você.<br/><br/>• ${stunIcon} <c:stun>Atordoamento</c:stun> - sem movimento, ataques ou habilidades, e a conjuração em andamento é interrompida<br/>• ${rootIcon} <c:control>Enraizamento</c:control> - preso no lugar e sem poder atacar, mas ainda pode conjurar habilidades<br/>• ${silenceIcon} <c:control>Silêncio</c:control> - sem habilidades, mas ainda pode mover e atacar<br/>• ${freezeIcon} <c:control>Congelamento</c:control> - igual ao enraizamento: sem movimento nem ataques, mas as habilidades funcionam<br/>• ${slowIcon} <c:slow>Lentidão</c:slow> - velocidade reduzida em vez de bloqueio total<br/>• ${fearIcon} <c:control>Medo</c:control> e <c:control>Encanto</c:control> - você continua se movendo, mas a direção é escolhida por outro, e seus ataques e habilidades ficam bloqueados<br/>• <c:control>Repulsão</c:control> - te lança ao ar e te atordoa durante toda a duração<br/>• <c:control>Puxão</c:control> - te arrasta e impede de agir<br/>• <c:control>Empurrão</c:control> - te empurra, mas você pode continuar atacando e conjurando<br/><br/><b>Atordoamento, Encanto e Medo</b> contam como <b>controle pesado</b>; efeitos e itens que falam de imunidade a controle pesado se referem a esses três.`,
        fr: `Le contrôle de foule est tout ce qui retire ses actions à un héros. La plupart peut être <c:dispel>dissipé</c:dispel>, et la <b>ténacité</b> raccourcit sa durée sur vous.<br/><br/>• ${stunIcon} <c:stun>Étourdissement</c:stun> - ni déplacement, ni attaque, ni compétence, et l'incantation en cours est interrompue<br/>• ${rootIcon} <c:control>Enracinement</c:control> - cloué sur place et incapable d'attaquer, mais vous pouvez encore incanter<br/>• ${silenceIcon} <c:control>Silence</c:control> - plus de compétences, mais déplacement et attaques restent possibles<br/>• ${freezeIcon} <c:control>Gel</c:control> - identique à l’enracinement : ni déplacement ni attaque, mais les compétences fonctionnent<br/>• ${slowIcon} <c:slow>Ralentissement</c:slow> - vitesse réduite plutôt qu'un blocage complet<br/>• ${fearIcon} <c:control>Peur</c:control> et <c:control>Charme</c:control> - vous continuez à bouger, mais la direction est choisie à votre place, et vos attaques comme vos compétences sont bloquées<br/>• <c:control>Repoussée</c:control> - vous projette en l’air et vous étourdit pendant toute sa durée<br/>• <c:control>Attraction</c:control> - vous traîne et vous empêche d’agir<br/>• <c:control>Poussée</c:control> - vous déplace, mais vous pouvez continuer à attaquer et à incanter<br/><br/><b>Étourdissement, Charme et Peur</b> comptent comme du <b>contrôle dur</b> ; les effets et objets parlant d'immunité au contrôle dur visent ces trois-là.`,
        zh: `控制效果是指任何剝奪英雄行動能力的效果。大多數都可以被<c:dispel>驅散</c:dispel>，而<b>韌性</b>能縮短它們在你身上的持續時間。<br/><br/>• ${stunIcon} <c:stun>暈眩</c:stun>——無法移動、攻擊或使用技能，且當前施法被中斷<br/>• ${rootIcon} <c:control>纏繞</c:control>——無法移動，也無法攻擊，但仍可施放技能<br/>• ${silenceIcon} <c:control>沉默</c:control>——無法使用技能，但仍可移動與攻擊<br/>• ${freezeIcon} <c:control>冰凍</c:control>——與纏繞相同：無法移動與攻擊，但技能仍可施放<br/>• ${slowIcon} <c:slow>減速</c:slow>——降低移動速度，而非完全禁錮<br/>• ${fearIcon} <c:control>恐懼</c:control>與<c:control>魅惑</c:control>——你仍會移動，但方向由他人決定，且攻擊與技能都被封鎖<br/>• <c:control>擊退</c:control>——將你擊飛到空中，並在持續時間內暈眩你<br/>• <c:control>拉扯</c:control>——把你拖走，期間無法行動<br/>• <c:control>推擠</c:control>——推開你，但你仍可攻擊與施法<br/><br/><b>暈眩、魅惑與恐懼</b>屬於<b>硬控制</b>；提到免疫硬控制的效果與裝備指的就是這三種。`,
        vi: `Khống chế là bất cứ thứ gì tước đi khả năng hành động của tướng. Phần lớn có thể bị <c:dispel>giải</c:dispel>, và <b>kiên cường</b> rút ngắn thời gian nó tác động lên bạn.<br/><br/>• ${stunIcon} <c:stun>Choáng</c:stun> - không di chuyển, không đánh, không dùng kỹ năng, và pha tung chiêu đang dở bị ngắt<br/>• ${rootIcon} <c:control>Trói chân</c:control> - bị ghim tại chỗ và không đánh thường được, nhưng vẫn tung chiêu được<br/>• ${silenceIcon} <c:control>Câm lặng</c:control> - không dùng được kỹ năng, nhưng vẫn di chuyển và đánh thường<br/>• ${freezeIcon} <c:control>Đóng băng</c:control> - giống trói chân: không di chuyển, không đánh thường, nhưng kỹ năng vẫn dùng được<br/>• ${slowIcon} <c:slow>Làm chậm</c:slow> - giảm tốc chạy thay vì khóa hoàn toàn<br/>• ${fearIcon} <c:control>Khiếp sợ</c:control> và <c:control>Mê hoặc</c:control> - bạn vẫn di chuyển, nhưng hướng đi do người khác quyết định, và cả đánh thường lẫn kỹ năng đều bị khóa<br/>• <c:control>Hất tung</c:control> - hất bạn lên không và gây choáng suốt thời gian đó<br/>• <c:control>Kéo</c:control> - lôi bạn đi và không cho hành động<br/>• <c:control>Đẩy lùi</c:control> - đẩy bạn đi, nhưng bạn vẫn đánh và tung chiêu được<br/><br/><b>Choáng, Mê hoặc và Khiếp sợ</b> được tính là <b>khống chế cứng</b>; các hiệu ứng và trang bị nói về miễn nhiễm khống chế cứng là nói tới ba loại này.`,
        id: `Crowd control adalah apa pun yang merampas aksi seorang hero. Sebagian besar bisa <c:dispel>di-dispel</c:dispel>, dan <b>tenacity</b> memperpendek durasinya padamu.<br/><br/>• ${stunIcon} <c:stun>Stun</c:stun> - tidak bisa bergerak, menyerang, atau memakai skill, dan cast yang sedang berjalan terputus<br/>• ${rootIcon} <c:control>Root</c:control> - terpaku di tempat dan tidak bisa menyerang, tapi masih bisa cast skill<br/>• ${silenceIcon} <c:control>Silence</c:control> - tidak bisa pakai skill, tapi masih bisa bergerak dan menyerang<br/>• ${freezeIcon} <c:control>Freeze</c:control> - sama seperti root: tidak bisa bergerak atau menyerang, tapi skill tetap bisa dipakai<br/>• ${slowIcon} <c:slow>Slow</c:slow> - kecepatan gerak berkurang, bukan terkunci sepenuhnya<br/>• ${fearIcon} <c:control>Fear</c:control> dan <c:control>Charm</c:control> - kamu tetap bergerak, tapi arahnya ditentukan orang lain, dan serangan maupun skill terkunci<br/>• <c:control>Knockback</c:control> - melontarkanmu ke udara dan membuatmu stun selama durasinya<br/>• <c:control>Pull</c:control> - menyeretmu dan membuatmu tidak bisa beraksi<br/>• <c:control>Pushback</c:control> - mendorongmu, tapi kamu tetap bisa menyerang dan cast<br/><br/><b>Stun, Charm dan Fear</b> dihitung sebagai <b>hard crowd control</b>; efek dan item yang menyebut kebal hard CC merujuk pada ketiganya.`,
        kr: `군중 제어란 챔피언에게서 행동을 빼앗는 모든 것을 말합니다. 대부분은 <c:dispel>정화</c:dispel>할 수 있고, <b>강인함</b>은 자신에게 걸린 지속 시간을 줄여 줍니다.<br/><br/>• ${stunIcon} <c:stun>기절</c:stun> - 이동, 공격, 스킬이 모두 불가하며 시전 중이던 스킬이 끊깁니다<br/>• ${rootIcon} <c:control>속박</c:control> - 제자리에 묶이고 공격도 할 수 없지만 스킬 시전은 가능합니다<br/>• ${silenceIcon} <c:control>침묵</c:control> - 스킬을 쓸 수 없지만 이동과 공격은 가능합니다<br/>• ${freezeIcon} <c:control>빙결</c:control> - 속박과 같습니다: 이동과 공격은 불가하지만 스킬은 사용할 수 있습니다<br/>• ${slowIcon} <c:slow>둔화</c:slow> - 완전한 봉쇄가 아니라 이동 속도 감소입니다<br/>• ${fearIcon} <c:control>공포</c:control>와 <c:control>매혹</c:control> - 계속 움직이지만 방향은 상대가 정하며 공격과 스킬이 모두 막힙니다<br/>• <c:control>넉백</c:control> - 공중으로 띄우며 지속 시간 동안 기절시킵니다<br/>• <c:control>끌어당기기</c:control> - 끌려가며 그동안 행동할 수 없습니다<br/>• <c:control>밀쳐내기</c:control> - 밀려나지만 공격과 시전은 계속할 수 있습니다<br/><br/><b>기절, 매혹, 공포</b>는 <b>강력한 군중 제어</b>로 분류되며, 하드 CC 면역을 말하는 효과와 아이템은 이 셋을 가리킵니다.`,
      }
    }
