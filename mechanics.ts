  ...
  switch (id) {
    ...
    case MechanicId.EXPERIENCE: {
      ...
      return {
        ...
        vi: `Kinh nghiệm được chia theo <b>khoảng cách</b>, không phải theo ai ra đòn cuối: bạn nhận được khi một lính địch chết trong <c:range>tầm ${range}</c:range> quanh bạn. Đứng quá xa thì không nhận được gì, đó là lý do rời đường tốn kém hơn nhiều so với số vàng bị bỏ lỡ.<br/><br/>• Một lính đáng giá <c:bonus>${minionExp} kinh nghiệm</c:bonus>, cộng <c:bonus>${perLevel}</c:bonus> cho mỗi cấp mà lính đó có<br/>• Một tướng địch đáng giá <c:bonus>${heroExp} kinh nghiệm</c:bonus>, cộng thưởng theo cấp độ và chuỗi hạ gục của họ<br/><br/>Tướng đạt tối đa <c:bonus>cấp ${MAX_LEVEL}</c:bonus>. Mỗi cấp tăng chỉ số cơ bản và mở khóa điểm thiên phú dọc đường.`,
    ...
    case MechanicId.CC_EFFECTS: {
      ...
      return {
        ...
        vi: `Khống chế là bất cứ thứ gì tước đi khả năng hành động của tướng. Phần lớn có thể bị <c:dispel>giải</c:dispel>, và <b>kiên cường</b> rút ngắn thời gian nó tác động lên bạn.<br/><br/>• ${stunIcon} <c:stun>Choáng</c:stun> - không di chuyển, không đánh, không dùng kỹ năng, và pha tung chiêu đang dở bị ngắt<br/>• ${rootIcon} <c:control>Trói chân</c:control> - bị ghim tại chỗ, nhưng vẫn đánh và tung chiêu được<br/>• ${silenceIcon} <c:control>Câm lặng</c:control> - không dùng được kỹ năng, nhưng vẫn di chuyển và đánh thường<br/>• ${freezeIcon} <c:control>Đóng băng</c:control> - đứng im hoàn toàn<br/>• ${slowIcon} <c:slow>Làm chậm</c:slow> - giảm tốc chạy thay vì khóa hoàn toàn<br/>• ${fearIcon} <c:control>Khiếp sợ</c:control> và <c:control>Mê hoặc</c:control> - bạn vẫn di chuyển, nhưng hướng đi do người khác quyết định và pha tung chiêu bị ngắt<br/>• <c:control>Hất tung</c:control>, <c:control>Đẩy lùi</c:control> và <c:control>Kéo</c:control> - di chuyển cưỡng bức kéo bạn khỏi vị trí đã chọn<br/><br/><b>Choáng, Mê hoặc và Khiếp sợ</b> được tính là <b>khống chế cứng</b>; các hiệu ứng và trang bị nói về miễn nhiễm khống chế cứng là nói tới ba loại này.`,
    ...
    case MechanicId.STATS: {
      ...
      return {
        ...
        vi: `Kháng tính theo một đường cong giảm dần hiệu quả, không phải trừ thẳng. <b>Giáp</b> giảm <c:physical>sát thương vật lý</c:physical> và <b>kháng phép</b> giảm <c:magical>sát thương phép</c:magical> theo cùng đường cong: <b>${lowArmor}</b> loại bỏ <b>${resist(lowArmor)}%</b>, còn <b>${midArmor}</b> loại bỏ <b>${resist(midArmor)}%</b>. <c:pure>Sát thương chuẩn</c:pure> bỏ qua cả hai.<br/><br/><b>Tốc biến kỹ năng</b> rút ngắn hồi chiêu theo đường cong riêng: <b>${lowHaste}</b> giúp kỹ năng hồi nhanh hơn <b>${haste(lowHaste)}%</b>, <b>${midHaste}</b> cho <b>${haste(midHaste)}%</b>. Khác với phần trăm cố định, nó không bao giờ chạm mốc không.<br/><br/><b>Tốc độ đánh</b> có sàn tại <b>${MIN_ATTACK_SPEED}</b>, một <b>đòn chí mạng</b> gây <b>+${critBonus}%</b> sát thương, và <b>kiên cường</b> rút ngắn hiệu ứng khống chế lên bạn.`,
    ...
    case MechanicId.VISION: {
      ...
      return {
        ...
        vi: `Bạn chỉ thấy những gì đội mình thắp sáng. Mọi thứ ngoài vùng đó đơn giản là không được vẽ ra - một kẻ địch đứng trong bóng tối là vô hình với bạn dù chẳng có gì che giấu hắn.<br/><br/>• Tướng của bạn mang <c:vision>tầm nhìn ${heroVision}</c:vision><br/>• Mỗi trụ chiếu sáng <c:vision>${towerVision}</c:vision> quanh nó<br/>• Lính của bạn thắp sáng đường khi đi qua<br/><br/><b>Bụi cỏ</b> che bạn khỏi bất kỳ ai đứng bên ngoài, đó là lý do chúng là nơi tự nhiên để mở giao tranh hoặc cắt tầm nhìn.<h2>Mắt</h2>Mắt mua tầm nhìn ở nơi bạn không có mặt. Cả hai đều là đồ tiêu hao, tàng hình ngay sau khi cắm và có thể bị phá khi bị phát hiện.<br/><br/>• ${observerIcon} <b>Mắt Quan Sát</b> - canh <c:vision>${wardVision}</c:vision> trong <b>${wardDuration} giây</b><br/>• ${sentryIcon} <b>Mắt Kiểm Soát</b> - canh <c:vision>${sentryVision}</c:vision> trong <b>${sentryDuration} giây</b> và cho <b>tầm nhìn thật</b><h2>Trang bị ảnh hưởng tầm nhìn</h2>• ${cloakIcon} <b>Áo Choàng Ánh Sáng</b> - nội tại cho <b>tầm nhìn thật</b> vĩnh viễn, và kỹ năng chủ động cắm miễn phí một mắt quan sát. Câu trả lời mạnh nhất cho kẻ địch tàng hình.<br/>• ${voidArmorIcon} <b>Giáp Hư Không</b> - kỹ năng chủ động khiến bạn <c:vision>tàng hình</c:vision> trong <b>${voidArmorDuration} giây</b> và tăng tốc, để mở giao tranh hoặc chạy thoát.<br/>• ${voidSwordIcon} <b>Kiếm Hư Không</b> - kỹ năng chủ động cắm một lưỡi kiếm chiếu sáng <c:vision>${voidSwordRadius}</c:vision> quanh nó khi còn tồn tại và tăng sức mạnh cho đồng minh bên trong: tầm nhìn bạn đặt ngay giữa trận đánh.<h2>Tàng hình</h2><c:vision>Tàng hình</c:vision>, từ bùa hoặc từ bộ kỹ năng của tướng, giấu bạn khỏi tầm nhìn thường. Hai thứ vẫn thấy bạn: <b>trụ phát hiện kẻ địch tàng hình trong tầm</b>, và <b>tầm nhìn thật</b> vẫn xuyên qua được.`,
