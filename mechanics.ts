...
const _getMechanicDescriptionLang = (id: MechanicId): { [key in string]: string } => {
  switch (id) {
    ...
    case MechanicId.STATS: {
      ...
      return {
        en: `<h2><img class="inline-mechanic-icon" src="/assets/mainpage/stats/armor.png" alt="" /> <c:armor>Armor</c:armor> and <img class="inline-mechanic-icon" src="/assets/mainpage/stats/magic_def.png" alt="" /> <c:bonus>Magic Resistance</c:bonus></h2><b>Armor</b> reduces <c:physical>physical damage</c:physical>. <b>Magic Resistance</b> reduces <c:magical>magical damage</c:magical>. Both use the same curve: higher stats reduce more damage, but each extra point adds less percentage reduction.<br/><br/><strong class="mechanic-formula">Damage reduced = (${RESISTANCE_CALCULATION_COEF} × R) ÷ (1 + ${RESISTANCE_CALCULATION_COEF} × ${RESISTANCE_CALCULATION_DIVISOR} × R)</strong>, where <b>R</b> is armor or Magic Resistance.<ul><li><b>${lowArmor} resistance:</b> <c:bonus>${resist(lowArmor)}% less damage</c:bonus>.</li><li><b>${midArmor} resistance:</b> <c:bonus>${resist(midArmor)}% less damage</c:bonus>.</li><li><c:pure>Pure damage</c:pure> ignores both.</li></ul>${resistanceChart}<h2><c:cooldown><i class="bx bx-time-five" aria-hidden="true"></i> Ability haste</c:cooldown></h2>Ability haste shortens cooldowns.<br/><br/><strong class="mechanic-formula">Cooldown = base × (1 - (${COOLDOWN_REDUCTION_CALCULATION_COEF} × H) ÷ (1 + ${COOLDOWN_REDUCTION_CALCULATION_COEF} × H))</strong>, where <b>H</b> is ability haste.<ul><li><b>${lowHaste} haste:</b> <c:bonus>${haste(lowHaste)}% shorter cooldowns</c:bonus>.</li><li><b>${midHaste} haste:</b> <c:bonus>${haste(midHaste)}% shorter cooldowns</c:bonus>.</li></ul>More haste keeps helping, but a cooldown never drops below <b>${minCooldown}%</b> of its base.<h2>Offence</h2><ul><li><img class="inline-mechanic-icon" src="/assets/mainpage/stats/attack_speed.png" alt="" /> <c:attackspeed>Attack speed:</c:attackspeed> how often you attack. <strong class="mechanic-formula">Time between attacks = ${attackInterval}s ÷ attack speed</strong>, and attack speed cannot fall below <b>${MIN_ATTACK_SPEED}</b>, or <b>${slowestAttack}s</b> per attack.</li><li><b>Critical hits:</b> deal <c:bonus>+${critBonus}% damage</c:bonus>, or <c:bonus>${critTotal}% of normal damage</c:bonus>.<br/>${katanaIcon} <b>Corrupted Katana</b> adds <c:bonus>${katanaBonus} percentage points</c:bonus>, raising critical hits to <c:bonus>${katanaCrit}% of normal damage</c:bonus>.</li><li><c:lifesteal>Lifesteal:</c:lifesteal> restores that share of the <c:physical>physical damage</c:physical> you deal. Only <c:bonus>a quarter</c:bonus> of it applies against minions and other non-heroes, and neither damage to buildings nor item effects heal you.</li></ul><h2>Survivability</h2><ul><li><img class="inline-mechanic-icon" src="/assets/mainpage/stats/health.png" alt="" /> <c:health>Health:</c:health> how much damage you survive. Every hero gains more of it each level - see their growth in the <b>Heroes</b> tab.</li><li><img class="inline-mechanic-icon" src="/assets/mainpage/stats/hpr.png" alt="" /> <c:heal>Health regeneration:</c:heal> health restored every second while you are alive. It scales with your level: <b>× ${regenAtLevel1}</b> at level 1, up to <b>× ${regenAtMaxLevel}</b> at level ${MAX_LEVEL}.</li><li><c:heal>Health restoration:</c:heal> health a heal gives back at once - from an ability, an item, the Healing Rune or standing in your own <b>Base</b>. It does not scale with your level, anything past your maximum health is wasted, and <c:magical>Grievous Wounds</c:magical> cuts it or removes it entirely.</li><li><c:shield>Shield:</c:shield> a pool that absorbs damage before your health does. Its size is fixed when the shield is applied, so anything that changes shield strength only counts at that moment.</li><li><c:tenacity>Tenacity:</c:tenacity> shortens crowd control duration applied to you by a percentage.</li></ul>${healthRegenChart}<h2><img class="inline-mechanic-icon" src="/assets/mainpage/stats/movespeed.png" alt="" /> <c:movespeed>Movement speed</c:movespeed></h2>How fast you run. Slows and speed boosts multiply rather than add up, and no hero moves faster than <b>${LIMIT_MOVEMENT_SPEED}</b>.`,
        ...
        vi: `<h2>Giáp và kháng phép</h2><b>Giáp</b> giảm <c:physical>sát thương vật lý</c:physical>. <b>Kháng phép</b> giảm <c:magical>sát thương phép</c:magical>. Cả hai dùng chung một đường cong: chỉ số càng cao giảm càng nhiều, nhưng mỗi điểm thêm vào lại đóng góp ít hơn.<br/><br/><b>Sát thương giảm = (${RESISTANCE_CALCULATION_COEF} × R) ÷ (1 + ${RESISTANCE_CALCULATION_COEF} × ${RESISTANCE_CALCULATION_DIVISOR} × R)</b>, với <b>R</b> là giáp hoặc kháng phép.<ul><li><b>${lowArmor} kháng:</b> <c:bonus>giảm ${resist(lowArmor)}% sát thương</c:bonus>.</li><li><b>${midArmor} kháng:</b> <c:bonus>giảm ${resist(midArmor)}% sát thương</c:bonus>.</li><li><c:pure>Sát thương chuẩn</c:pure> bỏ qua cả hai.</li></ul>${resistanceChart}<h2>Điểm hồi kỹ năng</h2>Điểm hồi kỹ năng rút ngắn thời gian hồi chiêu.<br/><br/><b>Hồi chiêu = gốc × (1 - (${COOLDOWN_REDUCTION_CALCULATION_COEF} × H) ÷ (1 + ${COOLDOWN_REDUCTION_CALCULATION_COEF} × H))</b>, với <b>H</b> là điểm hồi kỹ năng.<ul><li><b>${lowHaste} điểm hồi kỹ năng:</b> <c:bonus>hồi chiêu ngắn hơn ${haste(lowHaste)}%</c:bonus>.</li><li><b>${midHaste} điểm hồi kỹ năng:</b> <c:bonus>hồi chiêu ngắn hơn ${haste(midHaste)}%</c:bonus>.</li></ul>Càng nhiều điểm hồi kỹ năng vẫn càng có lợi, nhưng hồi chiêu không bao giờ thấp hơn <b>${minCooldown}%</b> giá trị gốc.<h2>Tấn công</h2><ul><li><b>Tốc độ đánh:</b> bạn đánh nhanh đến đâu. <b>Thời gian giữa hai đòn = ${attackInterval} giây ÷ tốc độ đánh</b>, và tốc độ đánh không thể thấp hơn <b>${MIN_ATTACK_SPEED}</b>, tức <b>${slowestAttack} giây</b> mỗi đòn.</li><li><b>Chí mạng:</b> gây <c:bonus>+${critBonus}% sát thương</c:bonus>, tức <c:bonus>${critTotal}% sát thương thường</c:bonus>.<br/>${katanaIcon} <b>Katana Tà Đạo</b> cộng thêm <c:bonus>${katanaBonus} điểm phần trăm</c:bonus>, nâng chí mạng lên <c:bonus>${katanaCrit}% sát thương thường</c:bonus>.</li><li><c:lifesteal>Hút máu:</c:lifesteal> hồi lại cho bạn phần đó của <c:physical>sát thương vật lý</c:physical> bạn gây ra. Với lính và các đơn vị không phải tướng chỉ còn <c:bonus>một phần tư</c:bonus>, còn sát thương lên công trình và hiệu ứng trang bị thì không hồi máu.</li></ul><h2>Khả năng sinh tồn</h2><ul><li><c:health>Máu:</c:health> bạn chịu được bao nhiêu sát thương. Mỗi tướng đều tăng máu theo từng cấp - xem mức tăng ở thẻ <b>Tướng</b>.</li><li><b>Hồi máu:</b> lượng máu hồi mỗi giây khi bạn còn sống, tăng theo cấp độ: <b>× ${regenAtLevel1}</b> ở cấp 1 và tới <b>× ${regenAtMaxLevel}</b> ở cấp ${MAX_LEVEL}.</li><li><b>Hồi phục máu:</b> lượng máu một hiệu ứng hồi trả lại ngay lập tức - từ kỹ năng, trang bị, Bùa Hồi Máu hoặc khi đứng trong <b>Căn Cứ</b> của bạn. Nó không tăng theo cấp độ, phần vượt quá máu tối đa bị lãng phí, và <c:healingreduction>Vết Thương Sâu</c:healingreduction> làm giảm hoặc vô hiệu hoàn toàn.</li><li><b>Khiên:</b> lớp đệm hấp thụ sát thương trước máu. Giá trị của nó được chốt ngay khi khiên được tạo, nên mọi hiệu ứng thay đổi độ mạnh của khiên chỉ tính vào thời điểm đó.</li><li><b>Kiên cường:</b> rút ngắn thời gian hiệu ứng khống chế lên bạn theo phần trăm.</li></ul>${healthRegenChart}<h2>Tốc độ di chuyển</h2>Bạn chạy nhanh đến đâu. Làm chậm và tăng tốc nhân với nhau chứ không cộng dồn, và không tướng nào di chuyển nhanh hơn <b>${LIMIT_MOVEMENT_SPEED}</b>.`,
...
const _getMechanicSectionDescriptionLang = (label: MechanicFigureLabel): { [key in string]: string } => {
  switch (label) {
    ...
    case MechanicFigureLabel.MINION_RANGED: {
      ...
      return {
        en: `Attacks from behind the melee minion.<ul><li><c:attackrange>${range} attack range</c:attackrange>.</li><li><b>Piercing shots</b> deal <b>${toMinion}% damage to minions</b>.</li></ul>Kill enemy ranged minions to help your wave push toward the enemy Base.`,
        ru: `Держится позади ближнего миньона и стреляет с <c:attackrange>дистанции ${range}</c:attackrange>. На бумаге у него самый низкий урон из троих, но его выстрел <b>пробивающий</b>: по другим миньонам он наносит <c:bonus>${toMinion}%</c:bonus>. Именно дальнобойные, а не ближние, решают, чья волна побеждает. Убивайте их первыми, если хотите, чтобы линия толкалась в вашу сторону.`,
        cz: `Drží se za jednotkou na blízko a střílí z <c:attackrange>vzdálenosti ${range}</c:attackrange>. Na papíře má nejnižší poškození ze všech tří, ale jeho střela je <b>probíjecí</b>: proti ostatním jednotkám způsobí <c:bonus>${toMinion}%</c:bonus>. Právě jednotky na dálku, ne ty na blízko, rozhodují o tom, čí vlna vyhraje. Zabíjej je první, když chceš, aby se linie tlačila tvým směrem.`,
        br: `Fica atrás do lacaio corpo a corpo e atira de <c:attackrange>${range} de alcance</c:attackrange>. No papel tem o menor dano dos três, mas o tiro dele é <b>perfurante</b>: causa <c:bonus>${toMinion}%</c:bonus> contra outros lacaios. São os lacaios à distância - e não os de corpo a corpo - que decidem qual onda vence. Mate-os primeiro quando quiser que a rota empurre para o seu lado.`,
        fr: `Reste derrière le sbire de mêlée et tire à <c:attackrange>${range} de portée</c:attackrange>. Sur le papier, il a les dégâts les plus faibles des trois, mais son tir est <b>perforant</b> : il inflige <c:bonus>${toMinion}%</c:bonus> aux autres sbires. Ce sont les sbires à distance, et non ceux de mêlée, qui décident quelle vague l'emporte. Tuez-les en premier si vous voulez que la voie pousse dans votre sens.`,
        zh: `跟在近戰小兵後方，從 <c:attackrange>${range} 範圍</c:attackrange>射擊。帳面上它是三者中傷害最低的，但它的射擊帶有<b>穿透</b>：對其他小兵造成 <c:bonus>${toMinion}%</c:bonus>。決定哪一波兵線獲勝的是遠程小兵，而非近戰小兵。若想讓兵線往你這側推進，請優先擊殺它們。`,
        vi: `Đứng sau lính cận chiến và bắn từ <c:attackrange>tầm ${range}</c:attackrange>. Trên giấy tờ nó có sát thương thấp nhất trong ba loại, nhưng phát bắn của nó <b>xuyên giáp</b>: gây <c:bonus>${toMinion}%</c:bonus> lên lính khác. Chính lính xạ chiến, chứ không phải lính cận chiến, quyết định đợt lính nào đẩy trụ. Hãy giết chúng trước nếu bạn muốn đẩy trụ về phía địch.`,
        id: `Berada di belakang minion jarak dekat dan menembak dari <c:attackrange>jangkauan ${range}</c:attackrange>. Di atas kertas damage-nya paling kecil dari ketiganya, tetapi tembakannya <b>menembus</b>: memberi <c:bonus>${toMinion}%</c:bonus> terhadap minion lain. Minion jarak jauh - bukan yang jarak dekat - yang menentukan gelombang mana yang menang. Bunuh mereka lebih dulu kalau kamu ingin lane mendorong ke arahmu.`,
        kr: `근접 미니언 뒤에 서서 <c:attackrange>사거리 ${range}</c:attackrange>에서 사격합니다. 수치상으로는 셋 중 피해량이 가장 낮지만 그 사격은 <b>관통</b> 속성이라 다른 미니언에게 <c:bonus>${toMinion}%</c:bonus>를 입힙니다. 어느 웨이브가 이기는지를 결정하는 것은 근접이 아니라 원거리 미니언입니다. 라인을 밀고 싶다면 이들을 먼저 처치하세요.`,
      }
    }
    case MechanicFigureLabel.MINION_SIEGE: {
      ...
      return {
        en: `Joins every <b>third wave</b>, starting with wave <b>7</b>. It has the most health and attacks from the back.<ul><li><c:attackrange>${range} attack range</c:attackrange>.</li><li>Bombs deal <b>${toTower}% damage to towers</b> and <b>${toHero}% to heroes</b>.</li><li>Drops <b>twice the gold</b>.</li></ul>Protect it to help destroy towers.`,
        ru: `Присоединяется к каждой третьей волне начиная с седьмой и идёт в самом конце, бросая бомбы с <c:attackrange>дистанции ${range}</c:attackrange>. Эти бомбы наносят <c:bonus>${toTower}%</c:bonus> башням, но лишь <b>${toHero}%</b> героям — это осадное оружие, для вас лично почти безобидное. Он же самый живучий миньон и даёт <b>вдвое больше золота</b>, поэтому осадные волны — тайминг, вокруг которого строят план обе команды.`,
        cz: `Připojuje se ke každé třetí vlně od sedmé a jde úplně vzadu, kde hází bomby z <c:attackrange>vzdálenosti ${range}</c:attackrange>. Ty bomby způsobí <c:bonus>${toTower}%</c:bonus> věžím, ale jen <b>${toHero}%</b> hrdinům – je to obléhací zbraň, pro tebe osobně téměř neškodná. Zároveň je to nejodolnější jednotka a padá z ní <b>dvojnásobek zlata</b>, proto obléhací vlny plánují obě strany.`,
        br: `Entra a cada terceira onda a partir da sétima e anda bem atrás, lançando bombas de <c:attackrange>${range} de alcance</c:attackrange>. Essas bombas causam <c:bonus>${toTower}%</c:bonus> em torres mas só <b>${toHero}%</b> em heróis, então é uma arma de cerco e quase inofensiva para você. Também é o lacaio mais resistente e larga <b>o dobro de ouro</b> - por isso as ondas de cerco são o tempo que os dois times planejam.`,
        fr: `Rejoint une vague sur trois à partir de la septième et marche tout au fond, lançant des bombes à <c:attackrange>${range} de portée</c:attackrange>. Ces bombes infligent <c:bonus>${toTower}%</c:bonus> aux tours mais seulement <b>${toHero}%</b> aux héros : c'est une arme de siège, presque inoffensive pour vous. C'est aussi le sbire le plus résistant et il lâche <b>le double d'or</b> - d'où le fait que les deux équipes planifient autour des vagues de siège.`,
        zh: `從第七波起每三波加入一次，走在最後方，從 <c:attackrange>${range} 範圍</c:attackrange>投擲炸彈。這些炸彈對防禦塔造成 <c:bonus>${toTower}%</c:bonus>，但對英雄僅 <b>${toHero}%</b>——它是攻城武器，對你本人幾乎無害。它同時是最耐打的小兵，並掉落<b>雙倍金幣</b>，這正是雙方都圍繞攻城波制定計畫的原因。`,
        vi: `Gia nhập mỗi ba đợt lính kể từ đợt thứ bảy và đi sau cùng, ném bom từ <c:attackrange>tầm ${range}</c:attackrange>. Những quả bom đó gây <c:bonus>${toTower}%</c:bonus> lên trụ nhưng chỉ <b>${toHero}%</b> lên tướng, nên đây là vũ khí công thành và gần như vô hại với cá nhân bạn. Nó cũng là lính trâu nhất và rơi <b>gấp đôi vàng</b> - đó là lý do cả hai đội đều tính toán quanh đợt lính công thành.`,
        id: `Bergabung setiap gelombang ketiga mulai dari yang ketujuh dan berjalan paling belakang, melempar bom dari <c:attackrange>jangkauan ${range}</c:attackrange>. Bom itu memberi <c:bonus>${toTower}%</c:bonus> ke menara tetapi hanya <b>${toHero}%</b> ke hero, jadi ia senjata pengepungan dan hampir tidak berbahaya bagimu. Ia juga minion paling tahan banting dan menjatuhkan <b>emas dua kali lipat</b> - itulah sebabnya kedua tim merencanakan sekitar gelombang siege.`,
        kr: `7번째 웨이브부터 세 웨이브마다 합류해 맨 뒤에서 걸으며 <c:attackrange>사거리 ${range}</c:attackrange>에서 폭탄을 던집니다. 이 폭탄은 타워에 <c:bonus>${toTower}%</c:bonus>를 주지만 챔피언에게는 <b>${toHero}%</b>만 들어가므로, 공성 병기일 뿐 여러분 개인에게는 거의 무해합니다. 또한 가장 단단한 미니언이며 <b>골드를 두 배</b>로 떨어뜨립니다. 양 팀이 공성 웨이브 타이밍을 중심으로 계획을 세우는 이유입니다.`,
      }
    }
    case MechanicFigureLabel.RUNE_DAMAGE: {
  ...
  switch (label) {
    case MechanicChartLabel.AXIS_RESISTANCE_OR_HASTE:
      return {
        ...
        zh: `護甲 / 魔法防禦 / 技能急速`,
        vi: `Giáp / Kháng phép / Điểm hồi kỹ năng`,
