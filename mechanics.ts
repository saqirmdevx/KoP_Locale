  ...
  switch (id) {
    case MechanicId.BASE: {
      ...
      return {
        ...
        vi: `Căn Cứ chính là cả trận đấu. Bảo vệ căn cứ của bạn và phá hủy căn cứ của địch - khoảnh khắc một bên sụp đổ, trận đấu kết thúc và đội đó thua. Mọi thứ khác trên bản đồ chỉ có ý nghĩa vì chúng đưa bạn tới gần điều đó.<br/><br/><c:dispel>Bạn không thể tự gây sát thương lên nó</c:dispel>. Nó miễn nhiễm vĩnh viễn với tất cả đòn đánh và kỹ năng, nên mọi sát thương của bạn đều không có tác dụng.<br/><br/><c:bonus>Chỉ lính mới hạ được nó.</c:bonus> Mỗi lính địch đi vào căn cứ sẽ tự hy sinh để gây sát thương, nghĩa là cần đúng <c:bonus>${minionsToDestroy} lính</c:bonus> tới được căn cứ để giành chiến thắng. Việc của bạn là hộ tống chúng tới đó.`,
    ...
    case MechanicId.TOWER: {
      ...
      return {
        ...
        vi: `Trụ có <c:health>${health} máu</c:health> và gây <c:pure>${damage} sát thương chuẩn</c:pure> trong <c:range>tầm ${range}</c:range>. Sát thương chuẩn bỏ qua hoàn toàn giáp và kháng phép, nên <b>không trang bị nào giảm được nó</b>.<br/><br/>Nó ưu tiên đánh lính, và chuyển sang tướng địch nào tấn công đồng minh trong tầm của nó - nhưng việc đổi mục tiêu này chỉ xảy ra tối đa mỗi <b>${aggroThrottle} giây</b>, nên trụ không nhảy qua lại giữa các mục tiêu.<br/><br/>Mỗi đòn đánh sẽ áp dụng hiệu ứng cộng dồn: mỗi lần cộng dồn tăng thêm <c:bonus>${stackBonus}%</c:bonus> sát thương trụ, tối đa <b>${stacks} lần cộng dồn</b>, mỗi lần cộng dồn kéo dài ${stackTime} giây. Các lần cộng dồn <c:dispel>không thể bị hóa giải</c:dispel>, nên bạn phải cẩn thận khi lao vào vì sát thương sẽ tăng dần.<br/><br/>Khi không có lính địch trong tầm, trụ nhận hiệu ứng <c:immune>Bảo Vệ Chống Đánh Lén</c:immune> và miễn nhiễm với mọi sát thương - bạn cần một đợt lính. Nó cũng <c:vision>phát hiện kẻ địch tàng hình</c:vision> và trụ nhận thêm sát thương từ lính công thành.<br/><br/>Phá hủy trụ địch thưởng cho đội bạn mỗi người ${GOLD_ICON} <c:bonus>${TOWER_KILL_GOLDS}</c:bonus> và vĩnh viễn tăng <c:bonus>${minionBonus}%</c:bonus> máu và sát thương cho lính của bạn.`,
    ...
    case MechanicId.MINIONS: {
      ...
      return {
        ...
        vi: `Lính là cách duy nhất để bạn gây sát thương lên căn cứ địch, nên mỗi đợt lính là một nguồn tài nguyên chứ không chỉ là vật cản.<br/><br/>Đợt đầu tiên xuất hiện lúc <b>0:00</b> và cứ mỗi <b>${waveTime} giây</b> lại có đợt lính tiếp theo: một lính cận chiến và một lính xạ chiến, cùng một lính công thành gia nhập mỗi ba đợt kể từ đợt thứ bảy. Sau khoảng hai mươi phút, một lính xạ chiến thứ hai gia nhập mỗi đợt.<br/><br/>Chúng mạnh lên theo thời gian trận đấu. Lính lên cấp mỗi <b>${levelEvery} đợt</b>, mỗi cấp cho <c:bonus>${levelBonus}% máu và sát thương</c:bonus>, tối đa tới cấp <b>${maxLevel}</b>. Khi trụ địch ở phía đó bị phá, chúng nhận thêm <c:bonus>${towerBonus}%</c:bonus> nữa.`,
    ...
    case MechanicId.GOLD: {
      ...
      return {
        ...
        vi: `Bạn bắt đầu với ${gold(INITIAL_GOLDS)} và nhận ${gold(GOLDS_PER_SEC)} mỗi giây trong suốt trận đấu.<br/><br/><b>Last hit</b> là nguồn thu chính: kết liễu một lính và bạn được ${gold(lastHit)} ngay lập tức (${coin} + ${BONUS_GOLD_PER_KILL_MINION} thưởng, cộng ${perLevel} cho mỗi cấp độ lính).<br/><br/>Nếu một lính chết mà <b>không</b> có tướng nào kết liễu - do trụ, nội tại hay một lính địch khác - nó sẽ rơi một <b>đồng vàng</b> ngay tại chỗ. Đồng vàng nằm đó trong <b>${coinLife} giây</b> và trị giá ${gold(coin)}; bất kỳ ai trong đội đã hạ gục nó đều có thể nhặt đồng vàng bằng cách đi qua nó. Lính công thành rơi hai đồng.<br/><br/>• Chiến Công Đầu thưởng cho người chơi đó ${gold(FIRST_BLOOD_GOLDS)}<br/>• Phá hủy trụ địch thưởng cho mỗi người chơi trong đội ${gold(TOWER_KILL_GOLDS)}`,
    ...
    case MechanicId.EXPERIENCE: {
      ...
      return {
        ...
        ru: `Опыт распределяется по <b>близости</b>, а не по тому, кто нанёс удар: вы получаете его, когда вражеский миньон умирает в пределах <c:range>${range} от вас</c:range>. Стоя слишком далеко, вы не получите ничего — поэтому уход с линии стоит больше, чем просто упущенное золото.<br/><br/>• Миньон даёт <c:bonus>${minionExp} опыта</c:bonus> плюс <c:bonus>${perLevel}</c:bonus> за каждый его уровень<br/>• Вражеский герой даёт <c:bonus>${heroExp} опыта</c:bonus> плюс бонусы за его уровень и серию убийств<br/><br/>Максимальный уровень героя — <c:bonus>${MAX_LEVEL}</c:bonus>. Каждый уровень повышает базовые характеристики и по пути открывает очки талантов.`,
        cz: `Zkušenosti se sdílejí podle <b>blízkosti</b>, ne podle toho, kdo zasadil ránu: získáš je pokaždé, když nepřátelská jednotka zemře v <c:range>dosahu ${range}</c:range> od tebe. Když stojíš příliš daleko, nedostaneš nic – proto opuštění linie stojí víc než jen ušlé zlato.<br/><br/>• Jednotka má hodnotu <c:bonus>${minionExp} zkušeností</c:bonus> plus <c:bonus>${perLevel}</c:bonus> za každou její úroveň<br/>• Nepřátelský hrdina má hodnotu <c:bonus>${heroExp} zkušeností</c:bonus> plus bonusy za jeho úroveň a sérii zabití<br/><br/>Hrdinové končí na <c:bonus>úrovni ${MAX_LEVEL}</c:bonus>. Každá úroveň zvedne základní statistiky a cestou odemyká body talentů.`,
        br: `A experiência é compartilhada por <b>proximidade</b>, não por quem deu o golpe: você a ganha sempre que um lacaio inimigo morre dentro de <c:range>${range} de alcance</c:range> de você. Ficar muito atrás não rende nada, e é por isso que sair da rota custa mais do que o ouro perdido.<br/><br/>• Um lacaio vale <c:bonus>${minionExp} de experiência</c:bonus>, mais <c:bonus>${perLevel}</c:bonus> por cada nível que ele tiver<br/>• Um herói inimigo vale <c:bonus>${heroExp} de experiência</c:bonus>, mais bônus pelo nível e pela sequência de abates<br/><br/>Heróis param no <c:bonus>nível ${MAX_LEVEL}</c:bonus>. Cada nível aumenta seus atributos base e libera pontos de talento ao longo do caminho.`,
        fr: `L'expérience se partage par <b>proximité</b>, pas selon qui a porté le coup : vous en gagnez dès qu'un sbire ennemi meurt dans un <c:range>rayon de ${range}</c:range> autour de vous. Rester trop en retrait ne rapporte rien, et c'est pourquoi quitter la voie coûte plus que l'or manqué.<br/><br/>• Un sbire vaut <c:bonus>${minionExp} d'expérience</c:bonus>, plus <c:bonus>${perLevel}</c:bonus> par niveau qu'il possède<br/>• Un héros ennemi vaut <c:bonus>${heroExp} d'expérience</c:bonus>, plus des bonus selon son niveau et sa série d'éliminations<br/><br/>Les héros plafonnent au <c:bonus>niveau ${MAX_LEVEL}</c:bonus>. Chaque niveau augmente vos statistiques de base et débloque des points de talent en chemin.`,
        zh: `經驗依<b>距離</b>共享，而非由誰造成最後一擊：只要敵方小兵死在你 <c:range>${range} 範圍</c:range>內，你就能獲得經驗。站得太遠則毫無所得，這也是離開線上的代價遠不只是損失金幣的原因。<br/><br/>• 一隻小兵價值 <c:bonus>${minionExp} 經驗</c:bonus>，並依其每一等級再加 <c:bonus>${perLevel}</c:bonus><br/>• 敵方英雄價值 <c:bonus>${heroExp} 經驗</c:bonus>，另有等級與連殺加成<br/><br/>英雄等級上限為 <c:bonus>${MAX_LEVEL} 級</c:bonus>。每一級都會提升基礎屬性，並在過程中解鎖天賦點數。`,
        vi: `Kinh nghiệm được chia theo <b>khoảng cách</b>, không phải theo ai ra đòn cuối: bạn nhận được khi một lính địch chết trong <c:range>tầm ${range}</c:range> quanh bạn. Đứng quá xa thì không nhận được gì, đó là lý do rời đường lính tốn kém hơn nhiều so với số vàng bị bỏ lỡ.<br/><br/>• Một lính đáng giá <c:bonus>${minionExp} kinh nghiệm</c:bonus>, cộng <c:bonus>${perLevel}</c:bonus> cho mỗi cấp lính<br/>• Một tướng địch khi hạ gục đáng giá <c:bonus>${heroExp} kinh nghiệm</c:bonus>, cộng thưởng theo cấp độ và chuỗi hạ gục của họ<br/><br/>Tướng đạt tối đa <c:bonus>cấp ${MAX_LEVEL}</c:bonus>. Mỗi cấp tăng chỉ số cơ bản và mở khóa điểm nhánh kỹ năng.`,
        id: `Pengalaman dibagi berdasarkan <b>kedekatan</b>, bukan siapa yang memukul terakhir: kamu mendapatkannya setiap kali minion musuh mati dalam <c:range>jangkauan ${range}</c:range> darimu. Berdiri terlalu jauh tidak menghasilkan apa-apa, itulah sebabnya meninggalkan lane lebih mahal daripada sekadar emas yang terlewat.<br/><br/>• Satu minion bernilai <c:bonus>${minionExp} pengalaman</c:bonus>, ditambah <c:bonus>${perLevel}</c:bonus> untuk setiap level yang dimiliki minion itu<br/>• Hero musuh bernilai <c:bonus>${heroExp} pengalaman</c:bonus>, ditambah bonus dari level dan killstreak mereka<br/><br/>Hero maksimal di <c:bonus>level ${MAX_LEVEL}</c:bonus>. Setiap level menaikkan statistik dasarmu dan membuka poin talenta di sepanjang jalan.`,
        kr: `경험치는 막타를 넣은 사람이 아니라 <b>거리</b>로 공유됩니다. 적 미니언이 자신으로부터 <c:range>${range} 범위</c:range> 안에서 죽으면 경험치를 얻습니다. 너무 멀리 서 있으면 아무것도 얻지 못하며, 라인을 비우는 비용이 놓친 골드보다 큰 이유가 바로 이것입니다.<br/><br/>• 미니언은 <c:bonus>경험치 ${minionExp}</c:bonus>에 미니언 레벨당 <c:bonus>${perLevel}</c:bonus>을 더한 값입니다<br/>• 적 챔피언은 <c:bonus>경험치 ${heroExp}</c:bonus>에 상대의 레벨과 연속 처치에 따른 보너스가 더해집니다<br/><br/>챔피언의 최대 레벨은 <c:bonus>${MAX_LEVEL}</c:bonus>입니다. 레벨마다 기본 능력치가 오르고 그 과정에서 특성 포인트가 열립니다.`,
      }
    }
    ...
    case MechanicId.SHOP: {
      ...
      return {
        ...
        vi: `Khu vực quanh căn cứ của bạn là vùng an toàn, được đánh dấu bằng hiệu ứng hạt màu xanh khi bạn đứng trong <c:range>bán kính ${radius}</c:range>.<br/><br/>Bên trong, bạn <c:heal>hồi ${healPerc}% máu tối đa</c:heal> mỗi ${tick} giây, mọi hiệu ứng bất lợi đều bị <c:dispel>gỡ bỏ</c:dispel>, và bạn nhận <c:pspeed>+${speed}% tốc chạy</c:pspeed> duy trì một lúc sau khi rời đi - chính điều này khiến việc về nhà nhanh là quan trọng.<br/><br/>Đứng trong căn cứ <b>địch</b> thì ngược lại bị trừng phạt: nó thiêu <c:pure>${BASE_ENEMY_PURE_DAMAGE} sát thương chuẩn</c:pure> mỗi ${tick} giây.<h2>Mua sắm</h2>Cửa hàng điều khiển bằng <b>chuột</b> và chỉ mở khi bạn đang đứng trong căn cứ của mình.<br/><br/>${inventory}<br/>Đưa con trỏ xuống <b>các ô trang bị ở đáy màn hình</b> - hàng đó vừa là túi trang bị vừa là cửa hàng. Rê chuột lên một ô để xem trang bị, và <b>bấm để mua</b>. Đồ bạn mua sẽ vào thẳng trong ô trống.<br/><br/><b>Bán lại.</b> Bán một món trong vòng <b>${refundWindow} giây kể từ khi mua</b> và bạn nhận lại <c:bonus>toàn bộ giá</c:bonus> - đủ để sửa một cú bấm nhầm. <b>Dùng món đó sẽ hủy ưu đãi này</b>: kích hoạt là đã tiêu giá trị của nó, nên giá bán tụt ngay về mức thường. Mọi lượt bán khác trả lại <b>${refundBack}%</b> giá gốc.`,
    ...
    case MechanicId.ITEMS: {
      ...
      return {
        ...
        vi: `Trang bị quan trong không kém gì cấp độ. Bạn mang tối đa <b>${slots}</b> món cùng lúc và chỉ mua được khi đang đứng trong căn cứ.<h2>Loại trang bị</h2>• ${potion} <b>Dùng một lần</b> - dùng một lần rồi <b>biến mất khỏi túi trang bị</b>. Bình Máu, Mắt Quan Sát và các món dùng một lần khác thuộc nhóm này. Chúng rẻ, nên hãy để trống một ô cho chúng.<br/>• ${boots} <b>Giày</b> - nhánh tốc chạy của bạn. Chúng nâng cấp thành nhiều loại chuyên biệt, nhưng bạn chỉ cần một đôi.<br/>• ${tier1} <b>Bậc 1</b> - các món nhỏ rẻ tiền một chỉ số.<br/>• ${tier2} <b>Bậc 2</b> - nâng cấp từ một món Bậc 1, thêm chỉ số thứ hai hoặc một nội tại.<br/>• ${tier3} <b>Bậc 3</b> - trang bị hoàn chỉnh, ghép từ Bậc 2. Đây là nơi có các kỹ năng chủ động và nội tại mạnh.<br/><br/>Vì mỗi bậc đều đi vào bậc sau, mua sớm một món nhỏ hiếm khi là phí vàng - nó sẽ thành một phần của trang bị hoàn chỉnh.<h2>Dùng trang bị</h2>Mỗi ô túi trang bị có <b>phím riêng</b>, bạn có thể đổi lại trong Cài đặt. Bấm phím của một ô sẽ dùng món nằm trong ô đó.<br/><br/>Phần lớn trang bị là <b>nội tại</b> - chỉ cho chỉ số khi mang theo. Một số là <b>chủ động</b>: ví dụ ${plateShield} <b>Khiên Giáp Tấm</b> cho giáp và máu khi mang, và bấm phím của nó sẽ kích hoạt kỹ năng với hồi chiêu <b>${plateCooldown} giây</b>. Bảng thông tin của trang bị cho biết nó có kỹ năng chủ động hay không và làm gì.<h2>Bán lại</h2>Bán một món trong vòng <b>${sec(ITEM_FULL_REFUND_WINDOW)} giây kể từ khi mua</b> sẽ hoàn <c:bonus>toàn bộ giá</c:bonus>. <b>Dùng món chủ động sẽ hủy điều đó</b> - khi đã tiêu hiệu ứng, món đó bán lại chỉ được <b>${perc(ITEM_SELL_REFUND_MULTIPLIER)}%</b> như mọi món khác.<br/><br/>Thẻ <b>Trang Bị</b> liệt kê mọi trang bị cùng giá và lộ trình ghép.`,
    ...
    case MechanicId.POWER_RUNES: {
      ...
      return {
        ...
        vi: `Ngọc Sức Mạnh xuất hiện ở phía trên bản đồ <b>vào phút đầu tiên của trận</b>, và cứ mỗi <b>${spawn} giây</b> lại có ngọc tiếp theo. Mỗi lúc chỉ có một loại ngọc, và nó kích hoạt ngay khi một tướng chạm vào - nên tranh giành nó là một trận đánh thật sự, không phải một món quà miễn phí.<br/><br/>Ngọc nào trong năm loại xuất hiện là ngẫu nhiên:`,
    ...
    case MechanicId.BOUNTY_RUNE: {
      ...
      return {
        ...
        vi: `Ngọc Vàng xuất hiện ở giữa phía dưới bản đồ mỗi <b>${spawn} giây</b>. Khác với Ngọc Sức Mạnh, nó không được nhặt - nó cần được <b>chiếm giữ</b>.<br/><br/>Đứng trong <c:range>bán kính ${radius}</c:range> để đẩy thanh chiếm giữ về phía đội bạn. Đội có <b>nhiều tướng hơn</b> trong vòng tròn sẽ tiến triển thanh; khi số lượng bằng nhau thanh đứng yên, và khi không có ai bên trong nó trôi dần về giữa. Mang thêm đồng đội <b>không</b> giúp chiếm nhanh hơn - chỉ thắng về số lượng. <c:bonus>Khoảng ${channel} giây kiểm soát không bị tranh chấp</c:bonus> là hoàn tất quá trình chiếm giữ.<br/><br/>Phần thưởng trao cho <b>người chơi trong đội chiếm giữ</b>, dù họ ở đâu: ${gold(GOLD_RUNE_BASE_GOLDS)} cộng ${gold(GOLD_RUNE_GOLDS_PER_MINUTE)} mỗi phút trôi qua, và <c:bonus>kinh nghiệm bằng ${expPerc}% số vàng đó</c:bonus>. Nó tăng theo thời gian trận đấu, nên Ngọc Vàng cuối trận có thể giá trị hơn một mạng.`,
    ...
    case MechanicId.KILLS: {
      ...
      return {
        ...
        vi: `Một mạng trả ${gold(base)} cộng <b>5%</b> tổng lượng vàng của tướng bị hạ gục, một điều chỉnh theo chênh lệch vàng giữa hai đội, và thưởng chuỗi hạ gục <b>${streak}</b> mỗi chuỗi (tối đa <b>${streakMax}</b>). Hạ một kẻ địch đang béo đáng giá hơn nhiều so với hạ một kẻ đang đói - và chết khi đang dẫn trước sẽ làm giá trị trở lại ban đầu.<br/><br/>Gây sát thương lên kẻ địch chết trong vòng <b>${assistWindow} giây</b> sẽ cho bạn một điểm <b>hỗ trợ</b>, trị giá <b>${assistGold}%</b> vàng và <b>${assistExp}%</b> kinh nghiệm.<br/><br/>Thời gian hồi sinh bằng <b>${respawnBase} giây</b> cộng <b>${respawnPerLevel} giây</b> mỗi cấp tướng, nên chết cuối trận phải trả giá đắt hơn nhiều so với đầu trận. Chết ở cấp ${MAX_LEVEL} khiến bạn vô hiệu hóa khoảng ${sec(RESPAWN_TIME_BASE + RESPAWN_TIME_PER_LEVEL * (MAX_LEVEL - 1))} giây - đủ lâu để mất một mục tiêu.`,
    ...
    case MechanicId.DAMAGE_TYPES: {
      ...
      return {
        ...
        vi: `Mọi đòn đánh trong trận đều mang một loại sát thương, và loại đó quyết định hệ số phòng thủ nào được áp dụng trước khi trừ bất kỳ con số nào.<br/><br/>• ${physicalIcon} <c:physical>Sát thương vật lý</c:physical> bị giảm bởi <b>giáp</b>, và chỉ gây <b>${toTower}%</b> lên trụ<br/>• ${magicalIcon} <c:magical>Sát thương phép</c:magical> bị giảm bởi <b>kháng phép</b><br/>• <c:pure>Sát thương chuẩn</c:pure> bỏ qua cả hai và <b>không thể bị giảm bởi bất cứ thứ gì</b><br/><br/>Còn ba loại nữa ở lính: <b>đòn đánh cận chiến</b> gây <b>${basicToHero}%</b> lên tướng, phát bắn <b>xuyên</b> của lính xạ chiến gây <b>${pierceToMinion}%</b> lên lính, và bom <b>công thành</b> gây <b>${siegeToTower}%</b> lên trụ nhưng chỉ <b>${siegeToHero}%</b> lên tướng địch.`,
    ...
    case MechanicId.CC_EFFECTS: {
      ...
      return {
        ...
        ru: `Контроль — это всё, что отнимает у героя возможность действовать. Большую его часть можно <c:dispel>снять</c:dispel>, а <b>стойкость</b> сокращает его длительность на вас.<br/><br/>• ${stunIcon} <c:stun>Оглушение</c:stun> — ни движения, ни атак, ни способностей, текущий каст прерывается<br/>• ${rootIcon} <c:control>Обездвиживание</c:control> — вы не можете двигаться, но атакуете и кастуете<br/>• ${silenceIcon} <c:control>Немота</c:control> — нет способностей, но движение и атаки доступны<br/>• ${freezeIcon} <c:control>Заморозка</c:control> — полная неподвижность<br/>• ${slowIcon} <c:slow>Замедление</c:slow> — снижение скорости передвижения, а не полная блокировка<br/>• ${fearIcon} <c:control>Страх</c:control> и <c:control>Очарование</c:control> — вы продолжаете двигаться, но направление выбирают за вас, а каст прерывается<br/>• <c:control>Отбрасывание</c:control>, <c:control>Толчок</c:control> и <c:control>Притягивание</c:control> — принудительное перемещение, уводящее вас с выбранной позиции<br/><br/><b>Оглушение, Очарование и Страх</b> считаются <b>жёстким контролем</b>; эффекты и предметы, дающие иммунитет к жёсткому контролю, имеют в виду именно их.`,
        cz: `Ovládání je všechno, co hrdinovi bere možnost jednat. Většinu lze <c:dispel>rozptýlit</c:dispel> a <b>houževnatost</b> zkracuje, jak dlouho na tebe působí.<br/><br/>• ${stunIcon} <c:stun>Omráčení</c:stun> – žádný pohyb, útoky ani schopnosti, a rozseslané kouzlo se přeruší<br/>• ${rootIcon} <c:control>Znehybnění</c:control> – nemůžeš se hýbat, ale pořád útočíš a sesíláš<br/>• ${silenceIcon} <c:control>Umlčení</c:control> – žádné schopnosti, ale pohyb i útok fungují<br/>• ${freezeIcon} <c:control>Zmrazení</c:control> – úplné znehybnění<br/>• ${slowIcon} <c:slow>Zpomalení</c:slow> – snížená rychlost pohybu, ne úplné zablokování<br/>• ${fearIcon} <c:control>Strach</c:control> a <c:control>Okouzlení</c:control> – dál se pohybuješ, ale směr za tebe vybírá někdo jiný a sesílání se přeruší<br/>• <c:control>Odhození</c:control>, <c:control>Odstrčení</c:control> a <c:control>Přitažení</c:control> – vynucený pohyb, který tě odtáhne z místa, které sis vybral<br/><br/><b>Omráčení, Okouzlení a Strach</b> se počítají jako <b>tvrdé ovládání</b>; efekty a předměty mluvící o imunitě vůči tvrdému ovládání myslí právě tyto tři.`,
        br: `Controle de grupo é tudo que tira as ações de um herói. A maior parte pode ser <c:dispel>dissipada</c:dispel>, e <b>tenacidade</b> encurta quanto tempo dura em você.<br/><br/>• ${stunIcon} <c:stun>Atordoamento</c:stun> - sem movimento, ataques ou habilidades, e a conjuração em andamento é interrompida<br/>• ${rootIcon} <c:control>Enraizamento</c:control> - preso no lugar, mas ainda pode atacar e conjurar<br/>• ${silenceIcon} <c:control>Silêncio</c:control> - sem habilidades, mas ainda pode mover e atacar<br/>• ${freezeIcon} <c:control>Congelamento</c:control> - completamente imóvel<br/>• ${slowIcon} <c:slow>Lentidão</c:slow> - velocidade reduzida em vez de bloqueio total<br/>• ${fearIcon} <c:control>Medo</c:control> e <c:control>Encanto</c:control> - você continua se movendo, mas a direção é escolhida por você e sua conjuração é interrompida<br/>• <c:control>Repulsão</c:control>, <c:control>Empurrão</c:control> e <c:control>Puxão</c:control> - movimento forçado que te arrasta do lugar escolhido<br/><br/><b>Atordoamento, Encanto e Medo</b> contam como <b>controle pesado</b>; efeitos e itens que falam de imunidade a controle pesado se referem a esses três.`,
        fr: `Le contrôle de foule est tout ce qui retire ses actions à un héros. La plupart peut être <c:dispel>dissipé</c:dispel>, et la <b>ténacité</b> raccourcit sa durée sur vous.<br/><br/>• ${stunIcon} <c:stun>Étourdissement</c:stun> - ni déplacement, ni attaque, ni compétence, et l'incantation en cours est interrompue<br/>• ${rootIcon} <c:control>Enracinement</c:control> - cloué sur place, mais vous pouvez encore attaquer et incanter<br/>• ${silenceIcon} <c:control>Silence</c:control> - plus de compétences, mais déplacement et attaques restent possibles<br/>• ${freezeIcon} <c:control>Gel</c:control> - totalement immobilisé<br/>• ${slowIcon} <c:slow>Ralentissement</c:slow> - vitesse réduite plutôt qu'un blocage complet<br/>• ${fearIcon} <c:control>Peur</c:control> et <c:control>Charme</c:control> - vous continuez à bouger, mais la direction est choisie à votre place et votre incantation est interrompue<br/>• <c:control>Repoussée</c:control>, <c:control>Poussée</c:control> et <c:control>Attraction</c:control> - déplacement forcé qui vous arrache à la position choisie<br/><br/><b>Étourdissement, Charme et Peur</b> comptent comme du <b>contrôle dur</b> ; les effets et objets parlant d'immunité au contrôle dur visent ces trois-là.`,
        zh: `控制效果是指任何剝奪英雄行動能力的效果。大多數都可以被<c:dispel>驅散</c:dispel>，而<b>韌性</b>能縮短它們在你身上的持續時間。<br/><br/>• ${stunIcon} <c:stun>暈眩</c:stun>——無法移動、攻擊或使用技能，且當前施法被中斷<br/>• ${rootIcon} <c:control>纏繞</c:control>——被定在原地，但仍可攻擊與施法<br/>• ${silenceIcon} <c:control>沉默</c:control>——無法使用技能，但仍可移動與攻擊<br/>• ${freezeIcon} <c:control>冰凍</c:control>——完全動彈不得<br/>• ${slowIcon} <c:slow>減速</c:slow>——降低移動速度，而非完全禁錮<br/>• ${fearIcon} <c:control>恐懼</c:control>與<c:control>魅惑</c:control>——你仍會移動，但方向由他人決定，且施法被中斷<br/>• <c:control>擊退</c:control>、<c:control>推擠</c:control>與<c:control>拉扯</c:control>——強制位移，把你拖離選定的位置<br/><br/><b>暈眩、魅惑與恐懼</b>屬於<b>硬控制</b>；提到免疫硬控制的效果與裝備指的就是這三種。`,
        vi: `Khống chế là bất cứ thứ gì tước đi khả năng hành động của tướng. Phần lớn có thể bị <c:dispel>giải</c:dispel>, và <b>kháng khống chế</b> rút ngắn thời gian nó tác động lên bạn.<br/><br/>• ${stunIcon} <c:stun>Choáng</c:stun> - Ngăn chặn di chuyển, tấn công, dùng kỹ năng, và tung chiêu đang kích hoạt bị dừng<br/>• ${rootIcon} <c:control>Trói chân</c:control> - bị ghim tại chỗ, nhưng vẫn đánh thường và tung chiêu được<br/>• ${silenceIcon} <c:control>Câm lặng</c:control> - không dùng được kỹ năng, nhưng vẫn di chuyển và đánh thường được<br/>• ${freezeIcon} <c:control>Đóng băng</c:control> - đứng im hoàn toàn<br/>• ${slowIcon} <c:slow>Làm chậm</c:slow> - giảm tốc chạy thay vì khóa chân hoàn toàn<br/>• ${fearIcon} <c:control>Khiếp sợ</c:control> và <c:control>Mê hoặc</c:control> - bạn vẫn di chuyển, nhưng hướng đi do người khác quyết định và khi tung chiêu bị dừng<br/>• <c:control>Hất tung</c:control>, <c:control>Đẩy lùi</c:control> và <c:control>Kéo</c:control> - cưỡng bức di chuyển kéo bạn khỏi vị trí đã di chuyển<br/><br/><b>Choáng, Mê hoặc và Khiếp sợ</b> được tính là <b>khống chế mạnh</b>; các hiệu ứng và trang bị nói về miễn nhiễm khống chế mạnh là nói tới ba loại này.`,
        id: `Crowd control adalah apa pun yang merampas aksi seorang hero. Sebagian besar bisa <c:dispel>di-dispel</c:dispel>, dan <b>tenacity</b> memperpendek durasinya padamu.<br/><br/>• ${stunIcon} <c:stun>Stun</c:stun> - tidak bisa bergerak, menyerang, atau memakai skill, dan cast yang sedang berjalan terputus<br/>• ${rootIcon} <c:control>Root</c:control> - terpaku di tempat, tapi masih bisa menyerang dan cast<br/>• ${silenceIcon} <c:control>Silence</c:control> - tidak bisa pakai skill, tapi masih bisa bergerak dan menyerang<br/>• ${freezeIcon} <c:control>Freeze</c:control> - membeku total di tempat<br/>• ${slowIcon} <c:slow>Slow</c:slow> - kecepatan gerak berkurang, bukan terkunci sepenuhnya<br/>• ${fearIcon} <c:control>Fear</c:control> dan <c:control>Charm</c:control> - kamu tetap bergerak, tapi arahnya ditentukan orang lain dan cast-mu terputus<br/>• <c:control>Knockback</c:control>, <c:control>Pushback</c:control> dan <c:control>Pull</c:control> - gerakan paksa yang menyeretmu dari posisi pilihanmu<br/><br/><b>Stun, Charm dan Fear</b> dihitung sebagai <b>hard crowd control</b>; efek dan item yang menyebut kebal hard CC merujuk pada ketiganya.`,
        kr: `군중 제어란 챔피언에게서 행동을 빼앗는 모든 것을 말합니다. 대부분은 <c:dispel>정화</c:dispel>할 수 있고, <b>강인함</b>은 자신에게 걸린 지속 시간을 줄여 줍니다.<br/><br/>• ${stunIcon} <c:stun>기절</c:stun> - 이동, 공격, 스킬이 모두 불가하며 시전 중이던 스킬이 끊깁니다<br/>• ${rootIcon} <c:control>속박</c:control> - 제자리에 묶이지만 공격과 시전은 가능합니다<br/>• ${silenceIcon} <c:control>침묵</c:control> - 스킬을 쓸 수 없지만 이동과 공격은 가능합니다<br/>• ${freezeIcon} <c:control>빙결</c:control> - 완전히 얼어붙어 움직일 수 없습니다<br/>• ${slowIcon} <c:slow>둔화</c:slow> - 완전한 봉쇄가 아니라 이동 속도 감소입니다<br/>• ${fearIcon} <c:control>공포</c:control>와 <c:control>매혹</c:control> - 계속 움직이지만 방향은 상대가 정하며 시전이 끊깁니다<br/>• <c:control>넉백</c:control>, <c:control>밀쳐내기</c:control>, <c:control>끌어당기기</c:control> - 고른 자리에서 강제로 끌려납니다<br/><br/><b>기절, 매혹, 공포</b>는 <b>강력한 군중 제어</b>로 분류되며, 하드 CC 면역을 말하는 효과와 아이템은 이 셋을 가리킵니다.`,
      }
    }
    ...
    case MechanicId.STATS: {
      ...
      return {
        ...
        ru: `Сопротивления работают по кривой с убывающей отдачей, а не простым вычитанием. <b>Броня</b> снижает <c:physical>физический урон</c:physical>, а <b>магическая защита</b> — <c:magical>магический</c:magical> по той же кривой: <b>${lowArmor}</b> убирает <b>${resist(lowArmor)}%</b>, а <b>${midArmor}</b> — уже <b>${resist(midArmor)}%</b>. <c:pure>Чистый урон</c:pure> игнорирует и то, и другое.<br/><br/><b>Ускорение способностей</b> сокращает перезарядки по своей кривой: <b>${lowHaste}</b> ускорения возвращает способности на <b>${haste(lowHaste)}%</b> быстрее, <b>${midHaste}</b> — на <b>${haste(midHaste)}%</b>. В отличие от плоского процента, оно никогда не достигает нуля.<br/><br/><b>Скорость атаки</b> ограничена снизу значением <b>${MIN_ATTACK_SPEED}</b>, <b>критический удар</b> наносит <b>+${critBonus}%</b> урона, а <b>стойкость</b> сокращает применённый к вам контроль.`,
        cz: `Odolnosti se řídí křivkou s klesajícím výnosem, ne prostým odečtem. <b>Brnění</b> snižuje <c:physical>fyzické poškození</c:physical> a <b>magická obrana</b> <c:magical>magické</c:magical> po stejné křivce: <b>${lowArmor}</b> odebere <b>${resist(lowArmor)}%</b>, zatímco <b>${midArmor}</b> odebere <b>${resist(midArmor)}%</b>. <c:pure>Čisté poškození</c:pure> ignoruje obojí.<br/><br/><b>Zrychlení schopností</b> zkracuje přebíjení po vlastní křivce: <b>${lowHaste}</b> zrychlení vrátí schopnosti o <b>${haste(lowHaste)}%</b> dřív, <b>${midHaste}</b> o <b>${haste(midHaste)}%</b>. Na rozdíl od plochého procenta nikdy nedosáhne nuly.<br/><br/><b>Rychlost útoku</b> má spodní hranici <b>${MIN_ATTACK_SPEED}</b>, <b>kritický zásah</b> způsobí <b>+${critBonus}%</b> poškození a <b>houževnatost</b> zkracuje ovládání použité na tebe.`,
        br: `As resistências seguem uma curva com retornos decrescentes, não uma subtração fixa. <b>Armadura</b> reduz <c:physical>dano físico</c:physical> e <b>defesa mágica</b> reduz <c:magical>dano mágico</c:magical> na mesma curva: <b>${lowArmor}</b> remove <b>${resist(lowArmor)}%</b>, enquanto <b>${midArmor}</b> remove <b>${resist(midArmor)}%</b>. <c:pure>Dano puro</c:pure> ignora ambos.<br/><br/><b>Aceleração de habilidade</b> encurta recargas na própria curva: <b>${lowHaste}</b> faz as habilidades voltarem <b>${haste(lowHaste)}%</b> mais rápido, <b>${midHaste}</b> faz <b>${haste(midHaste)}%</b>. Diferente de uma porcentagem fixa, nunca chega a zero.<br/><br/><b>Velocidade de ataque</b> tem piso em <b>${MIN_ATTACK_SPEED}</b>, um <b>acerto crítico</b> causa <b>+${critBonus}%</b> de dano, e <b>tenacidade</b> encurta o controle de grupo aplicado em você.`,
        fr: `Les résistances suivent une courbe à rendements décroissants, pas une soustraction fixe. <b>L'armure</b> réduit les <c:physical>dégâts physiques</c:physical> et la <b>défense magique</b> les <c:magical>dégâts magiques</c:magical> sur la même courbe : <b>${lowArmor}</b> en retire <b>${resist(lowArmor)}%</b>, tandis que <b>${midArmor}</b> en retire <b>${resist(midArmor)}%</b>. Les <c:pure>dégâts purs</c:pure> ignorent les deux.<br/><br/><b>L'accélération de compétences</b> raccourcit les recharges sur sa propre courbe : <b>${lowHaste}</b> fait revenir les compétences <b>${haste(lowHaste)}%</b> plus vite, <b>${midHaste}</b> donne <b>${haste(midHaste)}%</b>. Contrairement à un pourcentage fixe, elle n'atteint jamais zéro.<br/><br/>La <b>vitesse d'attaque</b> a un plancher à <b>${MIN_ATTACK_SPEED}</b>, un <b>coup critique</b> inflige <b>+${critBonus}%</b> de dégâts, et la <b>ténacité</b> raccourcit le contrôle qui vous est appliqué.`,
        zh: `抗性遵循邊際遞減曲線，而非固定減值。<b>護甲</b>減免<c:physical>物理傷害</c:physical>，<b>魔法抗性</b>減免<c:magical>魔法傷害</c:magical>，兩者使用同一條曲線：<b>${lowArmor}</b> 點可減免 <b>${resist(lowArmor)}%</b>，而 <b>${midArmor}</b> 點可減免 <b>${resist(midArmor)}%</b>。<c:pure>真實傷害</c:pure>則無視兩者。<br/><br/><b>技能急速</b>依自身曲線縮短冷卻：<b>${lowHaste}</b> 點急速使技能快 <b>${haste(lowHaste)}%</b> 回復，<b>${midHaste}</b> 點則為 <b>${haste(midHaste)}%</b>。與固定百分比不同，它永遠不會歸零。<br/><br/><b>攻擊速度</b>下限為 <b>${MIN_ATTACK_SPEED}</b>，<b>暴擊</b>造成 <b>+${critBonus}%</b> 傷害，<b>韌性</b>則縮短施加於你的控制效果。`,
        vi: `Giáp và kháng phép tính theo một đường cong giảm dần, không phải trừ thẳng. <b>Giáp</b> giảm <c:physical>sát thương vật lý</c:physical> và <b>kháng phép</b> giảm <c:magical>sát thương phép</c:magical> theo cùng đường cong: <b>${lowArmor}</b> loại bỏ <b>${resist(lowArmor)}%</b>, còn <b>${midArmor}</b> loại bỏ <b>${resist(midArmor)}%</b>. <c:pure>Sát thương chuẩn</c:pure> bỏ qua cả hai.<br/><br/><b>Thời gian hồi chiêu</b> rút ngắn hồi chiêu theo đường cong riêng: <b>${lowHaste}</b> giúp kỹ năng hồi nhanh hơn <b>${haste(lowHaste)}%</b>, <b>${midHaste}</b> cho <b>${haste(midHaste)}%</b>. Khác với phần trăm cố định, nó không bao giờ chạm mốc bằng không.<br/><br/><b>Tốc đánh</b> có sàn tại <b>${MIN_ATTACK_SPEED}</b>, một <b>đòn chí mạng</b> gây <b>+${critBonus}%</b> sát thương, và <b>kháng khống chế</b> rút ngắn hiệu ứng khống chế lên bạn.`,
        id: `Resistensi mengikuti kurva dengan hasil yang menurun, bukan pengurangan tetap. <b>Armor</b> mengurangi <c:physical>damage fisik</c:physical> dan <b>pertahanan sihir</b> mengurangi <c:magical>damage sihir</c:magical> pada kurva yang sama: <b>${lowArmor}</b> menghilangkan <b>${resist(lowArmor)}%</b>, sedangkan <b>${midArmor}</b> menghilangkan <b>${resist(midArmor)}%</b>. <c:pure>Damage murni</c:pure> mengabaikan keduanya.<br/><br/><b>Ability haste</b> memperpendek cooldown pada kurvanya sendiri: <b>${lowHaste}</b> haste membuat skill kembali <b>${haste(lowHaste)}%</b> lebih cepat, <b>${midHaste}</b> menjadi <b>${haste(midHaste)}%</b>. Tidak seperti persentase tetap, ia tidak pernah mencapai nol.<br/><br/><b>Kecepatan serang</b> memiliki batas bawah <b>${MIN_ATTACK_SPEED}</b>, <b>critical hit</b> memberi <b>+${critBonus}%</b> damage, dan <b>tenacity</b> memperpendek crowd control yang mengenaimu.`,
        kr: `저항력은 단순 차감이 아니라 수확 체감 곡선을 따릅니다. <b>방어력</b>은 <c:physical>물리 피해</c:physical>를, <b>마법 저항</b>은 <c:magical>마법 피해</c:magical>를 같은 곡선으로 줄입니다. <b>${lowArmor}</b>은 <b>${resist(lowArmor)}%</b>를, <b>${midArmor}</b>은 <b>${resist(midArmor)}%</b>를 제거합니다. <c:pure>고정 피해</c:pure>는 둘 다 무시합니다.<br/><br/><b>스킬 가속</b>은 자체 곡선으로 쿨다운을 줄입니다. 가속 <b>${lowHaste}</b>은 스킬이 <b>${haste(lowHaste)}%</b> 더 빨리 돌아오게 하고, <b>${midHaste}</b>은 <b>${haste(midHaste)}%</b>가 됩니다. 고정 퍼센트와 달리 결코 0에 도달하지 않습니다.<br/><br/><b>공격 속도</b>의 하한은 <b>${MIN_ATTACK_SPEED}</b>이며, <b>치명타</b>는 <b>+${critBonus}%</b>의 피해를 주고, <b>강인함</b>은 자신에게 걸린 군중 제어 시간을 줄입니다.`,
      }
    }
    ...
    case MechanicId.VISION: {
      ...
      return {
        ...
        ru: `Вы видите только то, что освещает ваша команда. Всё за пределами этого просто не отрисовывается — враг, стоящий в темноте, невидим для вас, даже если его ничто не скрывает.<br/><br/>• Ваш герой даёт <c:vision>${heroVision} обзора</c:vision><br/>• Каждая башня освещает <c:vision>${towerVision}</c:vision> вокруг себя<br/>• Миньоны освещают линию по мере продвижения<br/><br/><b>Кусты</b> скрывают вас от всех, кто стоит снаружи, — поэтому они естественное место для начала боя или разрыва линии видимости.<h2>Варды</h2>Варды покупают обзор там, где вас нет. Оба расходуемые, становятся невидимыми вскоре после установки и могут быть убиты, если их заметят.<br/><br/>• ${observerIcon} <b>Наблюдательный вард</b> — следит за <c:vision>${wardVision}</c:vision> в течение <b>${wardDuration} с</b><br/>• ${sentryIcon} <b>Сторожевой вард</b> — следит за <c:vision>${sentryVision}</c:vision> в течение <b>${sentryDuration} с</b> и даёт <b>истинное зрение</b><h2>Предметы, влияющие на обзор</h2>• ${cloakIcon} <b>Плащ света</b> — пассивно даёт постоянное <b>истинное зрение</b>, а активное умение бесплатно ставит наблюдательный вард. Лучший ответ невидимому врагу.<br/>• ${voidArmorIcon} <b>Броня пустоты</b> — активное умение делает вас <c:vision>невидимым</c:vision> на <b>${voidArmorDuration} с</b> и ускоряет, для захода или побега.<br/>• ${voidSwordIcon} <b>Меч пустоты</b> — активное умение втыкает клинок, который освещает <c:vision>${voidSwordRadius}</c:vision> вокруг себя и усиливает союзников рядом: обзор, который вы ставите прямо в бою.<h2>Невидимость</h2><c:vision>Невидимость</c:vision> от руны или из набора героя скрывает вас от обычного обзора. Два исключения: <b>башни обнаруживают невидимых врагов в радиусе</b>, а <b>истинное зрение</b> видит сквозь неё в любом случае.`,
        cz: `Vidíš jen to, co tvůj tým osvítí. Cokoli mimo se prostě nevykreslí – nepřítel stojící ve tmě je pro tebe neviditelný, i když ho nic neskrývá.<br/><br/>• Tvůj hrdina nese <c:vision>${heroVision} výhledu</c:vision><br/>• Každá věž osvětluje <c:vision>${towerVision}</c:vision> kolem sebe<br/>• Tvé jednotky osvětlují linii, jak po ní jdou<br/><br/><b>Keře</b> tě skryjí před každým, kdo stojí venku, což z nich dělá přirozené místo pro zahájení boje nebo přerušení linie pohledu.<h2>Wardy</h2>Wardy ti koupí výhled tam, kde nejsi. Oba jsou spotřební, krátce po položení se zneviditelní a jde je zabít, jakmile je někdo odhalí.<br/><br/>• ${observerIcon} <b>Pozorovací ward</b> – sleduje <c:vision>${wardVision}</c:vision> po <b>${wardDuration}s</b><br/>• ${sentryIcon} <b>Strážní ward</b> – sleduje <c:vision>${sentryVision}</c:vision> po <b>${sentryDuration}s</b> a dává <b>pravý zrak</b><h2>Předměty měnící výhled</h2>• ${cloakIcon} <b>Plášť světla</b> – pasivně nese trvalý <b>pravý zrak</b> a jeho aktivní schopnost položí ward zdarma. Nejsilnější odpověď na neviditelného nepřítele.<br/>• ${voidArmorIcon} <b>Brnění prázdnoty</b> – aktivní schopnost tě na <b>${voidArmorDuration}s</b> udělá <c:vision>neviditelným</c:vision> a zrychlí tě, na zahájení i útěk.<br/>• ${voidSwordIcon} <b>Meč prázdnoty</b> – aktivní schopnost zabodne čepel, která po dobu trvání osvětluje <c:vision>${voidSwordRadius}</c:vision> kolem sebe a posiluje spojence uvnitř: výhled, který pokládáš přímo v boji.<h2>Neviditelnost</h2><c:vision>Neviditelnost</c:vision> z runy nebo z hrdinovy výbavy tě skryje před běžným výhledem. Dvě věci tě přesto vidí: <b>věže odhalují neviditelné nepřátele v dosahu</b> a <b>pravý zrak</b> ji prohlédne tak jako tak.`,
        br: `Você só enxerga o que seu time ilumina. Qualquer coisa fora disso simplesmente não é desenhada - um inimigo parado no escuro é invisível para você mesmo sem nada o escondendo.<br/><br/>• Seu herói carrega <c:vision>${heroVision} de visão</c:vision><br/>• Cada torre ilumina <c:vision>${towerVision}</c:vision> ao seu redor<br/>• Seus lacaios iluminam a rota enquanto avançam<br/><br/><b>Arbustos</b> escondem você de quem estiver fora deles, o que os torna o lugar natural para iniciar uma luta ou quebrar a linha de visão.<h2>Sentinelas</h2>Sentinelas compram visão onde você não está. Ambas são consumíveis, ficam invisíveis logo após serem colocadas e podem ser destruídas quando avistadas.<br/><br/>• ${observerIcon} <b>Sentinela Observadora</b> - vigia <c:vision>${wardVision}</c:vision> por <b>${wardDuration}s</b><br/>• ${sentryIcon} <b>Sentinela de Controle</b> - vigia <c:vision>${sentryVision}</c:vision> por <b>${sentryDuration}s</b> e concede <b>visão verdadeira</b><h2>Itens que mudam a visão</h2>• ${cloakIcon} <b>Manto de Luz</b> - carrega <b>visão verdadeira</b> permanente como passiva, e seu ativo coloca uma sentinela grátis. A resposta mais forte contra um inimigo invisível.<br/>• ${voidArmorIcon} <b>Armadura do Vazio</b> - o ativo te deixa <c:vision>invisível</c:vision> por <b>${voidArmorDuration}s</b> e te acelera, para engajar ou fugir.<br/>• ${voidSwordIcon} <b>Espada do Vazio</b> - o ativo finca uma lâmina que ilumina <c:vision>${voidSwordRadius}</c:vision> ao redor enquanto dura, fortalecendo aliados dentro dela: visão que você coloca no meio da luta.<h2>Invisibilidade</h2><c:vision>Invisibilidade</c:vision>, da runa ou do kit de um herói, esconde você da visão normal. Duas coisas ainda te veem: <b>torres revelam inimigos invisíveis no alcance</b> e a <b>visão verdadeira</b> enxerga através dela.`,
        fr: `Vous ne voyez que ce que votre équipe éclaire. Tout le reste n'est simplement pas dessiné - un ennemi dans le noir vous est invisible même si rien ne le cache.<br/><br/>• Votre héros porte <c:vision>${heroVision} de vision</c:vision><br/>• Chaque tour éclaire <c:vision>${towerVision}</c:vision> autour d'elle<br/>• Vos sbires éclairent la voie au fil de leur avancée<br/><br/>Les <b>buissons</b> vous cachent de quiconque se tient à l'extérieur, ce qui en fait l'endroit naturel pour engager ou briser la ligne de vue.<h2>Balises</h2>Les balises achètent de la vision là où vous n'êtes pas. Toutes deux sont des consommables, deviennent invisibles peu après la pose et peuvent être détruites une fois repérées.<br/><br/>• ${observerIcon} <b>Balise d'observation</b> - surveille <c:vision>${wardVision}</c:vision> pendant <b>${wardDuration}s</b><br/>• ${sentryIcon} <b>Balise de contrôle</b> - surveille <c:vision>${sentryVision}</c:vision> pendant <b>${sentryDuration}s</b> et confère la <b>vision réelle</b><h2>Objets qui changent la vision</h2>• ${cloakIcon} <b>Cape de lumière</b> - porte la <b>vision réelle</b> permanente en passif, et son actif pose une balise gratuite. La meilleure réponse à un ennemi invisible.<br/>• ${voidArmorIcon} <b>Armure du vide</b> - son actif vous rend <c:vision>invisible</c:vision> pendant <b>${voidArmorDuration}s</b> et vous accélère, pour engager ou fuir.<br/>• ${voidSwordIcon} <b>Épée du vide</b> - son actif plante une lame qui éclaire <c:vision>${voidSwordRadius}</c:vision> autour d'elle et renforce les alliés à l'intérieur : de la vision que vous posez en plein combat.<h2>Invisibilité</h2><c:vision>L'invisibilité</c:vision>, de la rune ou du kit d'un héros, vous cache de la vision normale. Deux choses vous voient quand même : <b>les tours révèlent les ennemis invisibles à portée</b>, et la <b>vision réelle</b> la traverse.`,
        zh: `你只能看到己方點亮的區域。範圍之外的東西根本不會被繪製——站在黑暗中的敵人對你而言是隱形的，即使並沒有任何東西遮蔽他。<br/><br/>• 你的英雄擁有 <c:vision>${heroVision} 視野</c:vision><br/>• 每座防禦塔照亮周圍 <c:vision>${towerVision}</c:vision><br/>• 你的小兵在推進時會照亮兵線<br/><br/><b>草叢</b>能讓你對站在外面的人隱形，這正是它成為開戰或切斷視線最自然位置的原因。<h2>守衛</h2>守衛能在你不在的地方買到視野。兩者皆為消耗品，放置後不久即隱形，一旦被發現便可被摧毀。<br/><br/>• ${observerIcon} <b>偵查守衛</b>——監視 <c:vision>${wardVision}</c:vision>，持續 <b>${wardDuration} 秒</b><br/>• ${sentryIcon} <b>哨兵守衛</b>——監視 <c:vision>${sentryVision}</c:vision>，持續 <b>${sentryDuration} 秒</b>並提供<b>真實視野</b><h2>改變視野的裝備</h2>• ${cloakIcon} <b>光明斗篷</b>——被動提供永久<b>真實視野</b>，主動則免費放置一個偵查守衛。對付隱形敵人的最強解答。<br/>• ${voidArmorIcon} <b>虛空護甲</b>——主動使你<c:vision>隱身</c:vision> <b>${voidArmorDuration} 秒</b>並提升移速，可用於開戰或脫身。<br/>• ${voidSwordIcon} <b>虛空之劍</b>——主動插下一把劍，持續期間照亮周圍 <c:vision>${voidSwordRadius}</c:vision> 並強化範圍內隊友：這是你在戰鬥中臨時佈置的視野。<h2>隱身</h2>來自符文或英雄技能的<c:vision>隱身</c:vision>可讓你躲避一般視野。但有兩者仍能看見你：<b>防禦塔會偵測範圍內的隱身敵人</b>，而<b>真實視野</b>一律能看穿。`,
        vi: `Bạn chỉ thấy những gì đội mình thấy. Mọi thứ ngoài vùng đó đơn giản là không được vẽ ra - một kẻ địch đứng trong bóng tối là vô hình với bạn dù chẳng có gì che giấu hắn.<br/><br/>• Tướng của bạn mang <c:vision>tầm nhìn ${heroVision}</c:vision><br/>• Mỗi trụ cho tầm nhìn <c:vision>${towerVision}</c:vision> quanh nó<br/>• Lính của bạn cho tầm nhìn đường khi đi qua<br/><br/><b>Bụi cỏ</b> che bạn khỏi bất kỳ ai đứng bên ngoài, đó là lý do chúng là nơi tự nhiên để mở giao tranh hoặc cắt tầm nhìn.<h2>Mắt</h2>Mắt Quan Sát cho bạn lợi thế tầm nhìn ở nơi bạn không có mặt. Đều là trang bị tiêu hao, tàng hình ngay sau khi cắm và có thể bị phá khi bị phát hiện.<br/><br/>• ${observerIcon} <b>Mắt Quan Sát</b> - canh <c:vision>${wardVision}</c:vision> trong <b>${wardDuration} giây</b><br/>• ${sentryIcon} <b>Mắt Kiểm Soát</b> - canh <c:vision>${sentryVision}</c:vision> trong <b>${sentryDuration} giây</b> và cho <b>tầm nhìn thật</b><h2>Trang bị ảnh hưởng tầm nhìn</h2>• ${cloakIcon} <b>Áo Choàng Ánh Sáng</b> - nội tại cho <b>tầm nhìn xuyên thấu</b> vĩnh viễn, và kỹ năng chủ động cắm miễn phí một Mắt Quan Sát. Câu trả lời mạnh nhất cho kẻ địch tàng hình.<br/>• ${voidArmorIcon} <b>Giáp Hư Không</b> - kỹ năng kích hoạt khiến bạn <c:vision>tàng hình</c:vision> trong <b>${voidArmorDuration} giây</b> và tăng tốc, để mở giao tranh hoặc chạy thoát.<br/>• ${voidSwordIcon} <b>Kiếm Hư Không</b> - kỹ năng chủ động cắm một lưỡi kiếm chiếu sáng <c:vision>${voidSwordRadius}</c:vision> quanh nó khi còn tồn tại và tăng sức mạnh cho đồng minh bên trong: tầm nhìn bạn đặt ngay giữa trận đánh.<h2>Tàng hình</h2><c:vision>Tàng hình</c:vision>, từ ngọc hoặc từ bộ kỹ năng của tướng, giấu bạn khỏi tầm nhìn thường. Hai thứ vẫn thấy bạn: <b>trụ phát hiện kẻ địch tàng hình trong tầm</b>, và <b>tầm nhìn xuyên thấu</b> vẫn thấy được.`,
        id: `Kamu hanya melihat apa yang timmu terangi. Apa pun di luar itu tidak digambar sama sekali - musuh yang berdiri dalam gelap tak terlihat olehmu meski tidak ada yang menyembunyikannya.<br/><br/>• Hero-mu membawa <c:vision>penglihatan ${heroVision}</c:vision><br/>• Setiap menara menerangi <c:vision>${towerVision}</c:vision> di sekitarnya<br/>• Minion-mu menerangi lane saat mereka berjalan<br/><br/><b>Semak</b> menyembunyikanmu dari siapa pun yang berdiri di luarnya, itulah yang membuatnya tempat alami untuk memulai pertarungan atau memutus garis pandang.<h2>Ward</h2>Ward membeli penglihatan di tempat kamu tidak berada. Keduanya consumable, menjadi tak terlihat sesaat setelah dipasang, dan bisa dihancurkan begitu ketahuan.<br/><br/>• ${observerIcon} <b>Observer Ward</b> - mengawasi <c:vision>${wardVision}</c:vision> selama <b>${wardDuration} detik</b><br/>• ${sentryIcon} <b>Sentry Ward</b> - mengawasi <c:vision>${sentryVision}</c:vision> selama <b>${sentryDuration} detik</b> dan memberi <b>true sight</b><h2>Item yang mengubah penglihatan</h2>• ${cloakIcon} <b>Cloak of Light</b> - membawa <b>true sight</b> permanen sebagai pasif, dan aktifnya memasang observer ward gratis. Jawaban terkuat untuk musuh tak terlihat.<br/>• ${voidArmorIcon} <b>Void Armor</b> - aktifnya membuatmu <c:vision>tak terlihat</c:vision> selama <b>${voidArmorDuration} detik</b> dan mempercepatmu, untuk engage atau kabur.<br/>• ${voidSwordIcon} <b>Void Sword</b> - aktifnya menancapkan bilah yang menerangi <c:vision>${voidSwordRadius}</c:vision> di sekitarnya selama bertahan dan memperkuat sekutu di dalamnya: penglihatan yang kamu pasang di tengah pertarungan.<h2>Tak terlihat</h2><c:vision>Tak terlihat</c:vision>, dari rune atau dari kit hero, menyembunyikanmu dari penglihatan biasa. Dua hal tetap melihatmu: <b>menara mengungkap musuh tak terlihat dalam jangkauan</b>, dan <b>true sight</b> menembusnya.`,
        kr: `아군이 밝힌 곳만 볼 수 있습니다. 그 바깥은 아예 그려지지 않습니다. 어둠 속에 서 있는 적은 아무것도 그를 숨겨 주지 않아도 여러분에게는 보이지 않습니다.<br/><br/>• 챔피언은 <c:vision>시야 ${heroVision}</c:vision>을 가집니다<br/>• 타워는 주변 <c:vision>${towerVision}</c:vision>을 밝힙니다<br/>• 미니언은 라인을 따라 걸으며 시야를 밝힙니다<br/><br/><b>수풀</b>은 바깥에 선 상대로부터 자신을 숨겨 주며, 그래서 교전을 시작하거나 시야를 끊기에 가장 자연스러운 자리입니다.<h2>와드</h2>와드는 자신이 없는 곳의 시야를 사 줍니다. 둘 다 소모품이며, 설치 직후 은신하고, 발각되면 파괴될 수 있습니다.<br/><br/>• ${observerIcon} <b>감시 와드</b> - <b>${wardDuration}초</b> 동안 <c:vision>${wardVision}</c:vision>을 지켜봅니다<br/>• ${sentryIcon} <b>방어 와드</b> - <b>${sentryDuration}초</b> 동안 <c:vision>${sentryVision}</c:vision>을 지켜보며 <b>진시야</b>를 제공합니다<h2>시야를 바꾸는 아이템</h2>• ${cloakIcon} <b>빛의 망토</b> - 지속 효과로 영구 <b>진시야</b>를 가지며, 사용 시 감시 와드를 무료로 설치합니다. 은신한 적에 대한 가장 강력한 해답입니다.<br/>• ${voidArmorIcon} <b>공허의 갑옷</b> - 사용 시 <b>${voidArmorDuration}초</b> 동안 <c:vision>은신</c:vision>하고 이동 속도가 올라, 진입과 이탈 모두에 쓰입니다.<br/>• ${voidSwordIcon} <b>공허의 검</b> - 사용 시 검을 꽂아 지속되는 동안 주변 <c:vision>${voidSwordRadius}</c:vision>을 밝히고 그 안의 아군을 강화합니다. 교전 도중에 직접 놓는 시야입니다.<h2>은신</h2>룬이나 챔피언 스킬로 얻는 <c:vision>은신</c:vision>은 일반 시야에서 자신을 숨깁니다. 다만 두 가지는 여전히 봅니다. <b>타워는 사거리 내 은신한 적을 드러내고</b>, <b>진시야</b>는 그와 무관하게 꿰뚫어 봅니다.`,
      }
    }
  ...
  switch (label) {
    ...
    case MechanicFigureLabel.MINION_RANGED: {
      ...
      return {
        ...
        vi: `Đứng sau lính cận chiến và bắn từ <c:range>tầm ${range}</c:range>. Trên giấy tờ nó có sát thương thấp nhất trong ba loại, nhưng phát bắn của nó <b>xuyên giáp</b>: gây <c:bonus>${toMinion}%</c:bonus> lên lính khác. Chính lính xạ chiến, chứ không phải lính cận chiến, quyết định đợt lính nào đẩy trụ. Hãy giết chúng trước nếu bạn muốn đẩy trụ về phía địch.`,
    ...
    case MechanicFigureLabel.MINION_SIEGE: {
      ...
      return {
        ...
        vi: `Gia nhập mỗi ba đợt lính kể từ đợt thứ bảy và đi sau cùng, ném bom từ <c:range>tầm ${range}</c:range>. Những quả bom đó gây <c:bonus>${toTower}%</c:bonus> lên trụ nhưng chỉ <b>${toHero}%</b> lên tướng, nên đây là vũ khí công thành và gần như vô hại với cá nhân bạn. Nó cũng là lính trâu nhất và rơi <b>gấp đôi vàng</b> - đó là lý do cả hai đội đều tính toán quanh đợt lính công thành.`,
    ...
    case MechanicFigureLabel.RUNE_DAMAGE: {
      ...
      return {
        ...
        vi: `<c:bonus>+${dmgBonus}% sát thương vật lí</c:bonus> trong ${dmgDuration} giây. Ngọc đơn giản nhất trò chơi và nguy hiểm nhất khi vào tay một tướng vốn đã thắng thế trong giao tranh.`,
    ...
    case MechanicFigureLabel.RUNE_ARCANE: {
      ...
      return {
        ...
        vi: `<c:bonus>+${ARCANE_RUNE_BONUS_ABILITY_HASTE} hồi chiêu và +${ARCANE_RUNE_BONUS_ABILITY_POWER} sức mạnh phép</c:bonus> trong ${dmgDuration} giây - vừa tung được nhiều chiêu hơn vừa mạnh hơn.`,
    ...
    case MechanicFigureLabel.RUNE_HASTE: {
      ...
      return {
        ...
        vi: `Tốc chạy được đẩy lên mức cao nhất và <c:immune>miễn nhiễm làm chậm</c:immune> trong ${hasteDuration} giây. Đây là ngọc để đuổi hoặc chạy, không phải để đánh nhau.`,
    ...
    case MechanicFigureLabel.RUNE_HEALING: {
      ...
      return {
        ...
        vi: `Tạo vùng <c:range>bán kính ${HEALING_RUNE_EFFECT_RADIUS}</c:range> trong ${healDuration} giây, <c:heal>hồi ${healPerc}% máu tối đa</c:heal> mỗi ${healTick} giây và cho <c:pspeed>+${healSpeed}% tốc chạy</c:pspeed>. Nó chỉ giúp <b>đồng minh</b>, nên đây là ngọc duy nhất đáng để lại cho đồng đội đang hứng chịu sát thương.`,
