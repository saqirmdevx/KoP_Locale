    ...
    switch (itemId) {
        ...
        case Shared.ItemList.Iron_Basher:
            return {
                ...
                desc: {
                    en: `UNIQUE: Your normal attacks deal additional ${ItemAbilityData.SPIKED_MACE_DAMAGE} normal damage and apply griveous wound for ${toSec(ItemAbilityData.SPIKED_MACE_DURATION)}. (Griveous wound reduces target's all healing and regeneration effects by 65%). Cooldown ${toSec(ItemAbilityData.SPIKED_MACE_COOLDOWN)}`,
                    ...
                    zh: `特殊：你的一般攻擊給予額外 ${ItemAbilityData.SPIKED_MACE_DAMAGE}點一般傷害並給予 ${toSec(ItemAbilityData.SPIKED_MACE_DURATION)}創傷。(創傷降低目標物的所有治癒即回血效果 65%)。${toSec(ItemAbilityData.SPIKED_MACE_COOLDOWN)}冷卻時間`,
                }
            }
